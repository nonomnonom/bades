...
<AccordionGroup>
<Accordion title="What roles come with Bades.id by default?">
Bades.id comes with an **Admin** and **Member** roles by default. You can create additional custom roles based on your team's needs (e.g., Sales Rep, Manager, Read-Only User).
</Accordion>
...
You can control:
- **Send Email**: Ability to send emails from Bades.id
...
With JIT provisioning, user accounts are automatically created in Bades.id when someone logs in via SSO for the first time. They're assigned the default role automatically.
</Accordion>
