...
description: Step-by-step guide to migrate your data from any CRM to Bades.id.
---

## Overview

This guide walks you through migrating your data from any CRM to Bades.id. The process involves auditing your data, preparing your Bades.id workspace, exporting from your current system, and importing into Bades.id.
...
Create a mapping document between your current CRM and Bades.id:
...
## Step 3: Set Up Your Bades.id Workspace

Before importing data, prepare your Bades.id workspace:
...
If your data includes user references (Account Owner, Assignee, etc.), those users must exist in Bades.id before import. Otherwise, those relations cannot be mapped.
</Warning>
...
Open each exported CSV in a spreadsheet application and prepare it for Bades.id.

### Remove Duplicates

1. Sort by the unique field (email for People, domain for Companies)
2. Remove or merge duplicate rows
3. Verify no duplicates exist in Bades.id already
...
Use `https://domain.com` (not `domain.com` or `www.domain.com`). This matches Bades.id's format and prevents duplicates when you connect email/calendar sync.
</Warning>
...
1. Navigate to the object in Bades.id
...
- Bades.id will start syncing emails to contact records
...
<AccordionGroup>
  <Accordion title="Can I migrate workflows automatically?">
    Not currently. Workflows must be recreated manually in Bades.id.
  </Accordion>
...
Contact [contact@bades.id](mailto:contact@bades.id) or explore our [Implementation Services](/user-guide/getting-started/capabilities/implementation-services).
