...
description: Complete step-by-step guide to format your data for import into Bades.id.
---
...
**This is the most important step.** The sample file shows you the exact column names and format Bades.id expects.
...
Bades.id enforces uniqueness on certain fields. Duplicates will cause import errors.
...
3. Check for duplicates that already exist in Bades.id
...
- Must be unique (no duplicates in file or in Bades.id)
...
- Must be unique within your file and in Bades.id
...
### ID Fields
- **Optional**: Bades.id auto-generates IDs if not provided
...
- The parent record must already exist in Bades.id
...
## Step 6: Ensure Fields Exist in Bades.id
