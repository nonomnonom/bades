...
When you import a file containing a **unique identifier** that matches an existing record, Bades.id updates that record instead of creating a duplicate.

| If unique identifier... | Bades.id will... |
|-------------------------|----------------|
...
4. **Don't change unique identifier values** — or Bades.id will create new records
...
If you change `john@acme.com` to `john.smith@acme.com`, Bades.id will create a new record instead of updating the existing one.
</Warning>
...
Bades.id matches records by the unique identifier and updates them with new values.
...
3. Bades.id will match on that field
...
| **Case mismatch** | File has `John@acme.com` but Bades.id has `john@acme.com` | Creates new record | Export from Bades.id to get exact values |
...
<AccordionGroup>
  ...
  <Accordion title="How do I know which records will be updated vs. created?">
    During the import review step, Bades.id shows you how many records will be updated vs. created based on unique identifier matches.
  </Accordion>
