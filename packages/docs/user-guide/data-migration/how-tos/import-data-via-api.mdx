...
description: When and how to use Bades.id's APIs for large-scale data imports.
---

## Overview

Bades.id provides both **GraphQL** and **REST APIs** for programmatic data import. Use the API when CSV import isn't practical for your data volume or when you need automated, recurring imports.
...
Bades.id enforces rate limits to ensure system stability:
...
Bades.id supports two API types:
...
- Verify formats match Bades.id's requirements (see [Field Mapping](/user-guide/data-migration/capabilities/field-mapping))
...
- Verify data appears correctly in Bades.id
...
Contact us at [contact@bades.id](mailto:contact@bades.id) or explore our [Implementation Services](/user-guide/getting-started/capabilities/implementation-services).
