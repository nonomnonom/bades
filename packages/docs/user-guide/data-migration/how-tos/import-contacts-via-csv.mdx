...
description: Complete step-by-step guide to importing people/contacts into Bades.id.
---

## Overview

This guide walks you through importing your contacts (People) into Bades.id. **Import Companies first** if you want to link People to Companies.
...
- **Must be unique** — no duplicates in your file or in Bades.id
...
<Note>
**Use Domain URL format** (`https://acme.com`), not the label. This matches how Companies are stored in Bades.id.
</Note>
...
<Note>
**Pro tip:** Click **Download sample file** during import or export a few existing People to see the exact column names Bades.id expects.
</Note>
...
3. Wait for Bades.id to analyze your file

## Step 4: Map Your Columns

Bades.id automatically tries to match your columns to fields. Review and adjust:
...
3. Bades.id will link each Person to the matching Company
...
1. Bades.id shows your values alongside existing options
2. Match each value in your file to a Bades.id option
...
Before completing the import, Bades.id validates your data:
...
|-------|-------|----------|
| **Duplicate email** | Email already exists in Bades.id or file | Remove duplicate or update existing record |
...
Your contacts are now in Bades.id! Next steps:
...
2. Bades.id matches records by this unique identifier
...
<AccordionGroup>
  <Accordion title="Why can't two People have the same email?">
    Email is a unique identifier in Bades.id. This prevents duplicate contacts and ensures email sync correctly links emails to the right person.
  </Accordion>
  ...
  <Accordion title="Can I import the ID from my previous CRM?">
    Yes! Create a custom field marked as "unique" in your data model to store the external ID. Note: the field name `id` is reserved for Bades.id's internal ID.
  </Accordion>
