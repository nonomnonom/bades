...
description: Learn about the actions available in Bades.id workflows.
---
...
**Best Practice**: Use [branches](/user-guide/workflows/capabilities/workflow-branches) after Search Records to handle "found" vs "not found" scenarios.
### Upsert Record
...
<Note>
You can only send emails from mailboxes synced to your own Bades.id account. Sending from other team members' mailboxes (e.g., the account owner's email) is on the roadmap.
</Note>
...
<Tip>
**Working with arrays?** Arrays from external systems or previous steps may come as strings. See [How to handle arrays in Code actions](/user-guide/workflows/how-tos/advanced-configurations/handle-arrays-in-code-actions) for the solution.
</Tip>
...
<Note>
AI agents respect role-based permissions. You can assign specific roles to agents under **Settings → Members → Roles** to control what data they can access. See [Permissions](/user-guide/permissions-access/capabilities/permissions) for details.
</Note>
