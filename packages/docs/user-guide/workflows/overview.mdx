---
title: Workflows
description: Learn how to build automations in Bades.id.
---

## Why Workflows Matter

Bades.id was built to bring maximum flexibility to its users. Rather than forcing you to adapt your business processes to rigid, pre-built features, workflows enable you to build automations that create the CRM that best supports your unique business use cases.

Workflows are Bades.id's in-app feature for building these automations. They give you the building blocks to create exactly what your business needs, when it needs it.
...
2. **Bring data in and out of Bades.id**: Connect Bades.id via API calls and webhooks to your database and other tools.
...
- **Form**: Get inputs from users within Bades.id UI at the time of execution
