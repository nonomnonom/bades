...
Webhooks push data to your systems in real-time when events occur in Bades.id — no polling required. Use them to keep external systems in sync, trigger automations, or send alerts.
...
Bades.id sends webhooks for these event types:
...
Bades.id signs each webhook request for security. Validate signatures to ensure requests are authentic.

### Headers

| Header | Description |
|--------|-------------|
| `X-Bades.id-Webhook-Signature` | HMAC SHA256 signature |
| `X-Bades.id-Webhook-Timestamp` | Request timestamp |

### Validation Steps

1. Get the timestamp from `X-Bades.id-Webhook-Timestamp`
2. Create the string: `{timestamp}:{JSON payload}`
3. Compute HMAC SHA256 using your webhook secret
4. Compare with `X-Bades.id-Webhook-Signature`
...
| **Workflow Webhook Trigger** | IN | Receive data into Bades.id from external systems |
