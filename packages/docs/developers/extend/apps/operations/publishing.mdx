...
title: Publishing
icon: "upload"
description: Distribute your Bades.id app to the marketplace or deploy it internally.
---
...
- **Deploy a tarball** — upload your app directly to a specific Bades.id server for internal or private use.
- **Publish to npm** — list your app in the Bades.id marketplace for any workspace to discover and install.
...
For apps you don't want publicly available — proprietary tools, enterprise-only integrations, or experimental builds — you can deploy a tarball directly to a Bades.id server.
...
If your app uses a feature introduced in a specific Bades.id server version (for example, OAuth providers added in v2.3.0), you should declare the minimum server version your app requires using the `engines.twenty` field in `package.json`:
...
Apps generated with `create-bades-app` ship with two GitHub Actions workflows out of the box, under `.github/workflows/`. They are ready to run as soon as you push the repo to GitHub — no extra setup is needed for CI, and CD only requires a single secret.
...
2. Spawns an isolated Bades.id test instance using the `badesid/bades/.github/actions/spawn-twenty-app-dev-test@main` composite action (the CI equivalent of `yarn twenty docker:start --test`).
...
- `TWENTY_VERSION` (env, defaults to `latest`) — pin the Bades.id server version used in CI by editing this in `ci.yml`.
...
Deploys your app to a configured Bades.id server on every push to `main`, and optionally from a pull request when the `deploy` label is applied.

**What it does:**

1. Checks out the PR head (for labeled PRs) or the pushed commit.
2. Runs `badesid/bades/.github/actions/deploy-twenty-app@main` — the CI equivalent of `yarn twenty app:publish --private`.
3. Runs `badesid/bades/.github/actions/install-twenty-app@main` so the newly deployed version is installed into the target workspace.

**Required configuration:**

| Setting | Where | Purpose |
|---------|-------|---------|
| `TWENTY_DEPLOY_URL` | `env` in `cd.yml` (defaults to `http://localhost:3000`) | The Bades.id server to deploy to. Change this to your real server URL before first use. |
...
Both workflows reference reusable actions at `@main`, so action updates in the `badesid/bades` repo are picked up automatically. If you want deterministic builds, replace `@main` with a commit SHA or release tag on each `uses:` line.

## Publishing to npm

Publishing to npm makes your app discoverable in the Bades.id marketplace. Any Bades.id workspace can browse, install, and upgrade marketplace apps directly from the UI.

### Requirements

- An [npm](https://www.npmjs.com) account
- The `twenty-app` keyword in your `package.json` `keywords` array (add it manually — it is not included by default in the `create-bades-app` template)
...
The Bades.id server syncs its marketplace catalog from the npm registry **every hour**.
...
<Note>
If your app does not define an `aboutDescription` in `defineApplication()`, the marketplace will automatically use your package's `README.md` from npm as the about page content. This means you can maintain a single README for both npm and the Bades.id marketplace. If you want a different description in the marketplace, explicitly set `aboutDescription`.
</Note>
...
Go to the **Settings > Applications** page in Bades.id, where both marketplace and tarball-deployed apps can be browsed and installed.
