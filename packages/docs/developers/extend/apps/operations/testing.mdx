---
title: Testing
description: Vitest setup, integration tests against a real Bades.id server, type checking, and CI with GitHub Actions.
icon: "flask"
---

The SDK provides programmatic APIs that let you build, deploy, install, and uninstall your app from test code. Combined with [Vitest](https://vitest.dev/) and the typed API clients, you can write integration tests that verify your app works end-to-end against a real Bades.id server.
...
Make sure your local Bades.id server is running, then:
...
2. Spins up a temporary Bades.id server using the `badesid/bades/.github/actions/spawn-twenty-docker-image` action
  ...
  test:
    ...
    steps:
      ...
      - name: Spawn Bades.id instance
        id: twenty
        uses: badesid/bades/.github/actions/spawn-twenty-docker-image@main
        with:
...
You don't need to configure any secrets — the `spawn-twenty-docker-image` action starts an ephemeral Bades.id server directly in the runner and outputs the connection details. The `GITHUB_TOKEN` secret is provided automatically by GitHub.

To pin a specific Bades.id version instead of `latest`, change the `TWENTY_VERSION` environment variable at the top of the workflow.
