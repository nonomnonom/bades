...
- Use `serverVariables` (not `applicationVariables`) — OAuth credentials are server-wide and one OAuth app per Bades.id server.
...
4. Open the installed app in Bades.id as a server admin → set the values on the corresponding `serverVariables`.
