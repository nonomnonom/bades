...
Relations connect two objects together. In Bades.id, relations are always **bidirectional** — every relation has two sides, and each side is declared as a field that references the other.
...
To create a relation with a built-in Bades.id object (Person, Company, etc.), use `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
