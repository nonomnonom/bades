...
title: Quick Start
icon: "rocket"
description: Create your first Bades.id app in minutes.
---
...
- **Docker** — [Download](https://www.docker.com/products/docker-desktop/). Needed to run a local Bades.id server. Skip if you already have Bades.id running elsewhere.

Building a Bades.id app has three phases. The scaffolder collapses them into one happy-path command, but each phase is a separate concept — when something fails, knowing which phase you're in tells you what to fix.

| Phase | What you do | Tool | Result |
|---|---|---|---|
| **1. Scaffold** | Generate the app's source code | `npx create-bades-app` | A TypeScript project on disk |
| **2. Run a server** | Start a Bades.id server to sync into | Docker + `yarn twenty server` | A running Bades.id instance |
...
## Phase 2 — Run a local Bades.id server

Your app needs a Bades.id server to sync into. The server is a full Bades.id instance — UI, GraphQL API, PostgreSQL — running locally in Docker. Your local code uploads its definitions to that server, which makes them appear in the UI.

The scaffolder offers to start one for you:

> **Would you like to set up a local Bades.id instance?**
...
- **No** — choose this if you already have a Bades.id server you want to connect to. You can wire it up later with `yarn twenty remote:add`.
...
<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Bades.id login screen" />
</div>

Click **Authorize** on the next screen — this gives the CLI access to your workspace.

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Bades.id CLI authorization screen" />
</div>
...
**After this phase:** you have a running Bades.id server at [http://localhost:2020](http://localhost:2020) with your CLI authorized to sync to it.
...
| **Front components** | React components that render inside Bades.id's UI (side panel, widgets, command menu) |
