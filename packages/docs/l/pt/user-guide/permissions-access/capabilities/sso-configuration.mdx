...
O Single Sign-On (SSO) permite que os membros da sua equipe façam login no Bades.id usando o provedor de identidade da sua organização. Isso oferece:
...
O Bades.id oferece suporte a SSO com:
...
* Acesso de administrador ao workspace do Bades.id

<Note>
**Para usuários de auto-hospedagem que desejam configurar SSO**, entre em contato pelo contact@bades.id
</Note>
...
* **ID da Entidade**: Fornecido pelo Bades.id
* **URL ACS**: A URL de retorno para autenticação
* **Certificado**: Para comunicação segura

#### 4. Insira os detalhes do provedor no Bades.id
...
* Remova-os do workspace do Bades.id
...
* **Mapeamento de grupos**: Mapeie os grupos do IdP para as funções do Bades.id (se disponível)
* **Processo de desligamento**: Inclua o Bades.id no seu fluxo de trabalho de desprovisionamento
