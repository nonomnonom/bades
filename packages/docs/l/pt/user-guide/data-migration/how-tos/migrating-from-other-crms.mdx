...
description: Guia passo a passo para migrar os seus dados de qualquer CRM para o Bades.id.
---

## Visão Geral

Este guia explica como migrar os seus dados de qualquer CRM para o Bades.id. O processo envolve auditar os seus dados, preparar o seu espaço de trabalho no Bades.id, exportar do seu sistema atual e importar para o Bades.id.
...
Crie um documento de mapeamento entre o seu CRM atual e o Bades.id:
...
* O campo de destino no Bades.id
...
## Passo 3: Configure o seu espaço de trabalho no Bades.id

Antes de importar dados, prepare o seu espaço de trabalho no Bades.id:
...
Se os seus dados incluírem referências a utilizadores (Proprietário da conta, Responsável, etc.), esses utilizadores devem existir no Bades.id antes da importação. Caso contrário, essas relações não podem ser mapeadas.
</Warning>
...
Abra cada CSV exportado numa aplicação de folha de cálculo e prepare-o para o Bades.id.

### Remova duplicados

1. Ordene pelo campo único (e-mail para Pessoas, domínio para Empresas)
2. Remova ou una linhas duplicadas
3. Verifique se não existem já duplicados no Bades.id
...
Use `https://domain.com` (não `domain.com` nem `www.domain.com`). Isto corresponde ao formato do Bades.id e evita duplicados quando ligar a sincronização de e-mail/calendário.
</Warning>
...
2. Use os mesmos endereços de e-mail que os utilizadores usaram para aderir ao seu espaço de trabalho no Bades.id
...
## Passo 6: Importe para o Bades.id
...
1. Navegue até ao objeto no Bades.id
...
* O Bades.id começará a sincronizar e-mails com os registos de contactos
...
<AccordionGroup>
  <Accordion title="Posso migrar fluxos de trabalho automaticamente?">
    De momento, não. Os fluxos de trabalho devem ser recriados manualmente no Bades.id.
  </Accordion>
...
Contacte [contact@bades.id](mailto:contact@bades.id) ou explore os nossos [Serviços de Implementação](/l/pt/user-guide/getting-started/capabilities/implementation-services).
