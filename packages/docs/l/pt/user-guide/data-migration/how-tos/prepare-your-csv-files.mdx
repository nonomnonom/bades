...
description: Guia completo passo a passo para formatar seus dados para importação no Bades.id.
---
...
**Esta é a etapa mais importante.** O arquivo de exemplo mostra os nomes exatos das colunas e o formato que o Bades.id espera.
...
O Bades.id impõe unicidade em determinados campos. Duplicados causarão erros de importação.
...
3. Verifique se há duplicados que já existem no Bades.id
...
* Deve ser exclusivo (sem duplicados no arquivo ou no Bades.id)
...
* Deve ser exclusivo dentro do seu arquivo e no Bades.id
...
* **Links / Rótulo do Link**: `Bades.id`
* **Links / URL do Link**: `https://bades.id`
...
* **Opcional**: a Bades.id gera IDs automaticamente se não forem fornecidos
...
* O registro pai já deve existir na Bades.id
...
## Etapa 6: Garanta que os campos existam na Bades.id
