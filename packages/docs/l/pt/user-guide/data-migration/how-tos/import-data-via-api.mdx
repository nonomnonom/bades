...
description: Quando e como usar as APIs da Bades.id para importações de dados em larga escala.
---

## Visão Geral

A Bades.id oferece tanto **GraphQL** como **APIs REST** para a importação programática de dados. Use a API quando a importação por CSV não for prática para o seu volume de dados ou quando precisar de importações automatizadas e recorrentes.
...
A Bades.id impõe limites de taxa para assegurar a estabilidade do sistema:
...
A Bades.id suporta dois tipos de API:
...
* Verifique se os formatos correspondem aos requisitos da Bades.id (veja [Mapeamento de Campos](/l/pt/user-guide/data-migration/capabilities/field-mapping))
...
* Verifique se os dados aparecem corretamente na Bades.id
...
Contacte-nos em [contact@bades.id](mailto:contact@bades.id) ou explore os nossos [Serviços de Implementação](/l/pt/user-guide/getting-started/capabilities/implementation-services).
