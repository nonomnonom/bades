...
O Bades.id suporta a importação de relações entre objetos durante a importação por CSV. Isto permite-lhe ligar registos (por exemplo, associar Pessoas a Empresas) como parte da sua migração de dados.
...
## Como funcionam as relações no Bades.id

### Um para Muitos / Muitos para Um

O Bades.id suporta relações padrão em que um registo se liga a muitos outros:
...
4. O Bades.id ligará automaticamente cada registo ao pai correspondente
...
<Note>**Ligação a Membros do Espaço de Trabalho**: Quando a relação aponta para Membros do Espaço de Trabalho (a sua equipa que inicia sessão no Bades.id), refira-os pelo seu **endereço de email**, não pelo nome.</Note>
