---
title: Glossário
description: Familiarize-se com a terminologia essencial usada no Bades.id.
---

## API

API (Interface de Programação de Aplicações) permite que você conecte o Bades.id com outros sistemas de software e construa integrações personalizadas.

## Aplicativos

Os aplicativos são extensões personalizadas escritas em código que podem definir modelos de dados e funções de lógica. Eles permitem que desenvolvedores criem personalizações reutilizáveis que podem ser implantadas em vários espaços de trabalho.
...
Oportunidades no Bades.id CRM são potenciais negócios ou vendas com contas ou contatos.
...
Tarefas no Bades.id CRM são atividades atribuídas relacionadas a contatos, contas ou oportunidades.
...
Webhooks são mensagens automáticas enviadas do Bades.id a outras aplicações quando eventos específicos ocorrem, permitindo a sincronização de dados em tempo real.
...
Um `Espaço de Trabalho` normalmente representa uma empresa usando o Bades.id. Ele contém todos os registros e dados que você e os membros da sua equipe adicionam ao Bades.id.
...
Membros do Espaço de Trabalho são os usuários do Bades.id da sua equipe que têm acesso ao seu espaço de trabalho. Eles podem ser designados como proprietários ou responsáveis por registros.
