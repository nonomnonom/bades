...
description: Defina objetos, campos, papéis e metadados da aplicação com o Bades.id SDK.
icon: database
---
...
<Note>
**Os campos base são criados automaticamente.** Quando você define um objeto personalizado, o Bades.id adiciona automaticamente campos padrão
...
Use `defineField()` para adicionar campos a objetos que não são seus — como objetos padrão do Bades.id (Person, Company, etc.). ou a objetos de outros apps. Ao contrário dos campos inline em `defineObject()`, os campos independentes exigem um `objectUniversalIdentifier` para especificar qual objeto eles estendem:
...
As relações conectam objetos entre si. No Bades.id, as relações são sempre **bidirecionais** — você define ambos os lados, e cada lado faz referência ao outro.
...
Para criar uma relação com um objeto integrado do Bades.id (Person, Company, etc.), use `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
