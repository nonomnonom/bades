...
* Use `serverVariables` (não `applicationVariables`) — as credenciais OAuth são do servidor como um todo e há um app OAuth por servidor do Bades.id.
...
4. Abra o app instalado no Bades.id como administrador do servidor → defina os valores nos `serverVariables` correspondentes.
