---
title: Arquitetura
description: Como as aplicações Bades.id funcionam — sandboxing, ciclo de vida e os blocos de construção.
icon: sitemap
---

As aplicações Bades.id são pacotes TypeScript que estendem seu espaço de trabalho com objetos personalizados, lógica, componentes de UI e recursos de IA. Elas são executadas na plataforma Bades.id com sandboxing completo e controles de permissão.
...
| **Componente de front-end**   | UI React em sandbox na página do Bades.id               | [Componentes de front-end](/l/pt/developers/extend/apps/front-components) |
...
* **Componentes de front-end** executam em Web Workers usando Remote DOM — isolados da página principal, mas renderizando elementos DOM nativos (não iframes). Eles se comunicam com o Bades.id por meio de uma API de host com passagem de mensagens.
...
* **`yarn twenty dev`** — observa seus arquivos-fonte e sincroniza ao vivo as alterações com um servidor Bades.id conectado. O cliente de API tipado é regenerado automaticamente quando o esquema muda.
...
<CardGroup cols={2}>
  ...
  <Card title="Componentes de front-end" icon="window-maximize" href="/l/pt/developers/extend/apps/front-components">
    Componentes React em sandbox dentro da UI do Bades.id.
  </Card>
