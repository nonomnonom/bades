...
As funções de lógica são funções TypeScript no lado do servidor que são executadas na plataforma Bades.id. Elas podem ser acionadas por solicitações HTTP, agendamentos cron ou eventos de banco de dados — e também podem ser expostas como ferramentas para agentes de IA.
...
* **`toolTriggerSettings`** — torna a função disponível para os recursos de IA do Bades.id (chat, MCP, chamadas de função). Usa o JSON Schema padrão, o formato que os LLMs entendem nativamente.
* **`workflowActionTriggerSettings`** — torna a função visível como uma etapa no construtor visual de fluxos de trabalho. Usa o `InputSchema` avançado do Bades.id para que o construtor possa renderizar editores de campo adequados, seletores de variáveis e rótulos.
...
* `toolTriggerSettings.inputSchema` e `workflowActionTriggerSettings.inputSchema` são opcionais. Quando omitidos, o construtor de manifestos os infere a partir do código-fonte do handler (JSON Schema para a ferramenta de IA, `InputSchema` do Bades.id para a ação de fluxo de trabalho). Forneça um explicitamente quando quiser uma tipagem mais rica — por exemplo, com campos compatíveis com `FieldMetadataType`, como `CURRENCY` ou `RELATION` para o construtor de fluxos de trabalho, ou com campos `description` que o agente de IA pode ler:
...
* As variáveis de ambiente `APPLICATION_ID`, `APP_ACCESS_TOKEN` e `API_URL` estão disponíveis dentro do manipulador (assim como em qualquer outra função de lógica), então você pode chamar a API da Bades.id com um token de acesso de aplicativo com escopo para o seu app.
...
O pacote `twenty-client-sdk` fornece dois clientes GraphQL tipados para interagir com a API do Bades.id a partir das suas funções de lógica e componentes de front-end.
...
<Note>
  Quando seu código é executado no Bades.id (funções de lógica ou componentes de front-end), a plataforma injeta credenciais como variáveis de ambiente:

  * `TWENTY_API_URL` — URL base da API do Bades.id
