...
description: Crie componentes React que renderizam dentro da UI do Bades.id com isolamento em sandbox.
icon: window-maximize
---

Componentes de front-end são componentes React que renderizam diretamente dentro da UI do Bades.id. Eles são executados em um Web Worker isolado usando Remote DOM — seu código é sandboxed, mas renderiza nativamente na página, não em um iframe.

## Onde os componentes de front-end podem ser usados

Os componentes de front-end podem ser renderizados em dois locais dentro do Bades.id:
...
Como o componente retorna `null`, o Bades.id ignora renderizar um contêiner para ele — nenhum espaço vazio aparece no layout. O componente ainda tem acesso a todos os hooks e à API de comunicação do host.
...
| Variável                  | Descrição                                                     |
| ------------------------- | ------------------------------------------------------------- |
| `TWENTY_API_URL`          | URL base da API da Bades.id                                     |
...
* **Componentes de UI do Bades.id** — importe de `twenty-sdk/ui` (Button, Tag, Status, Chip, Avatar e mais)
