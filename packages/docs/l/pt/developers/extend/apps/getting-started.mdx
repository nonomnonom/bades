...
icon: rocket
description: Crie seu primeiro app do Bades.id em minutos.
---
...
* **Docker** — [Baixar](https://www.docker.com/products/docker-desktop/). Necessário para executar um servidor Bades.id local. Ignore se você já tiver o Bades.id em execução em outro lugar.

A criação de um aplicativo Bades.id tem três fases. A ferramenta de scaffolding as reúne em um único comando do fluxo ideal, mas cada fase é um conceito separado — quando algo falha, saber em que fase você está indica o que corrigir.

| Fase                        | O que você faz                                     | Ferramenta                    | Resultado                             |
| --------------------------- | -------------------------------------------------- | ----------------------------- | ------------------------------------- |
| **1. Criar scaffolding**    | Gerar o código-fonte do aplicativo                 | `npx create-bades-app`       | Um projeto TypeScript em disco        |
| **2. Executar um servidor** | Iniciar um servidor Bades.id para o qual sincronizar | Docker + `yarn twenty server` | Uma instância Bades.id em execução      |
...
## Fase 2 — Executar um servidor Bades.id local

Seu aplicativo precisa de um servidor Bades.id para o qual sincronizar. O servidor é uma instância completa do Bades.id — interface, API GraphQL, PostgreSQL — executando localmente no Docker. Seu código local envia suas definições para esse servidor, o que faz com que elas apareçam na interface.

A ferramenta de scaffolding oferece iniciar um para você:

> **Você gostaria de configurar uma instância local do Bades.id?**
...
* **Não** — escolha isto se você já tiver um servidor Bades.id ao qual deseja se conectar. Você pode conectá-lo depois com `yarn twenty remote add`.
...
<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Tela de login do Bades.id" />
</div>

Clique em **Authorize** na próxima tela — isso dá à CLI acesso ao seu espaço de trabalho.

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Tela de autorização da CLI do Bades.id" />
</div>
...
**Após esta fase:** você tem um servidor Bades.id em execução em [http://localhost:2020](http://localhost:2020) com sua CLI autorizada a sincronizar com ele.
...
<Warning>
O modo de desenvolvimento só está disponível em instâncias do Bades.id em modo de desenvolvimento (`NODE_ENV=development`). Instâncias de produção rejeitam solicitações de sincronização de desenvolvimento — use `yarn twenty deploy` para implantar em servidores de produção. Veja [Publicação de aplicativos](/l/pt/developers/extend/apps/publishing).
</Warning>
...
| **Componentes de front-end** | Componentes React que são renderizados na UI do Bades.id (painel lateral, widgets, menu de comandos)                 |
...
Use `yarn twenty server` para controlar o contêiner Bades.id local:
...
Travou? Peça ajuda no [Discord da Bades.id](https://discord.com/channels/1130383047699738754/1130386664812982322).
