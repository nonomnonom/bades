...
title: Publicação
icon: carregar
description: Distribua seu aplicativo Bades.id no Marketplace ou implante-o internamente.
---
...
* **Implantar um tarball** — envie seu aplicativo diretamente para um servidor Bades.id específico para uso interno ou privado.
* **Publicar no npm** — liste seu aplicativo no Marketplace da Bades.id para que qualquer espaço de trabalho possa descobrir e instalar.
...
Para aplicativos que você não quer disponibilizar publicamente — ferramentas proprietárias, integrações apenas para empresas ou builds experimentais — você pode implantar um tarball diretamente em um servidor Bades.id.
...
Se o seu aplicativo usar um recurso introduzido em uma versão específica do servidor Bades.id (por exemplo, provedores OAuth adicionados na v2.3.0), você deve declarar a versão mínima do servidor que seu aplicativo requer usando o campo `engines.twenty` em `package.json`:
...
Os apps gerados com `create-bades-app` já vêm com dois fluxos de trabalho do GitHub Actions prontos, em `.github/workflows/`. Eles estão prontos para executar assim que você fizer push do repositório para o GitHub — nenhuma configuração extra é necessária para CI, e CD requer apenas um único segredo.
...
2. Inicia uma instância de teste do Bades.id isolada usando a ação composta `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` (o equivalente em CI de `yarn twenty server start --test`).
...
* `TWENTY_VERSION` (env, padrão `latest`) — fixe a versão do servidor Bades.id usada no CI editando isto em `ci.yml`.
...
Faz o deploy do seu app para um servidor Bades.id configurado a cada push para `main` e, opcionalmente, a partir de um pull request quando o rótulo `deploy` é aplicado.
...
| ----------------------- | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `env` em `cd.yml` (padrão `http://localhost:3000`)                   | O servidor Bades.id para o qual fazer o deploy. Altere isto para a URL real do seu servidor antes do primeiro uso. |
...
Publicar no npm torna seu aplicativo descobrível no Marketplace da Bades.id. Qualquer espaço de trabalho da Bades.id pode navegar, instalar e atualizar aplicativos do Marketplace diretamente pela UI.

### Requisitos

* Uma conta no [npm](https://www.npmjs.com)
* A palavra-chave `twenty-app` no array `keywords` do seu `package.json` (adicione-a manualmente — não é incluída por padrão no template `create-bades-app`)
...
O servidor Bades.id sincroniza seu catálogo do marketplace a partir do registro do npm **a cada hora**.
...
Vá para a página **Configurações > Aplicações** no Bades.id, onde é possível navegar e instalar tanto apps do marketplace quanto apps implantados por tarball.
