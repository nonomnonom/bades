---
title: Aplicativos Bades.id
description: Crie e gerencie personalizações do Bades.id como código.
---
...
Os aplicativos permitem que você estenda o Bades.id com objetos, campos, funções de lógica, componentes de interface, habilidades de IA e mais — tudo gerenciado como código. Em vez de configurar tudo pela UI, você define seu modelo de dados e a lógica em TypeScript e implanta em um ou mais workspaces.
...
* **Componentes de interface** — componentes React que são exibidos na UI do Bades.id (páginas de registro, menu de comandos, painéis laterais)
* **Habilidades e agentes de IA** — estenda a IA do Bades.id com recursos personalizados
...
Isso cria a estrutura de um novo aplicativo, inicia opcionalmente um servidor Bades.id local e começa a monitorar seus arquivos por alterações. Veja o guia [Primeiros passos](/l/pt/developers/extend/apps/getting-started) para o passo a passo completo.
...
| `defineFrontComponent()`           | Componentes React na UI do Bades.id                        |
...
3. **`yarn twenty deploy`** — implanta em um servidor Bades.id remoto
