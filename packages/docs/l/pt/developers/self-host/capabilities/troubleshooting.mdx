...
Se você tiver uma instância existente do Bades.id com dados em produção, **NÃO** siga estes passos, pois eles excluirão permanentemente seu banco de dados!

Ao instalar o Bades.id pela primeira vez, você pode querer alterar a senha padrão do banco de dados.
...
Durante a instalação do Bades.id, é necessário configurar seu banco de dados postgres com os esquemas, extensões e usuários corretos.
...
**Se não funcionar:**
Investigue quais processos estão consumindo a maior parte da RAM do seu computador. No Bades.id, percebemos que algumas extensões do VScode estavam consumindo muita RAM, então as desativamos temporariamente.
...
* Abra seu aplicativo Bades.id, navegue até `/settings`, e ative a alternância **Avançado** na parte inferior esquerda da tela.
* Crie um novo webhook.
* Cole **Sua URL Webhook Única** no campo **Endpoint Url** no Bades.id. Defina os **Filtros** para `Companies` e `Created`.
...
Se você está executando o Bades.id por trás de um proxy reverso e está enfrentando problemas de conexão:
...
2. **Verifique as configurações do Proxy Reverso:**
   ...
   * Confirme que seu proxy reverso está encaminhando corretamente as solicitações para o servidor Bades.id.
   * Certifique-se de que cabeçalhos como `X-Forwarded-For` e `X-Forwarded-Proto` estão configurados corretamente.

3. **Reinicie os Serviços:**

   Após fazer as alterações, reinicie tanto o proxy reverso quanto os contêineres do Bades.id.
...
* Suporte Comunitário:
  ...
  Entre em contato com a [comunidade Bades.id](https://github.com/twentyhq/twenty/issues) ou [canais de suporte](https://discord.gg/cx5n4Jzs57) para obter assistência.
