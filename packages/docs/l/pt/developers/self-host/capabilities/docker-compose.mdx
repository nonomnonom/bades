...
Este guia fornece instruções passo a passo para instalar e configurar o aplicativo Bades.id usando Docker Compose. O objetivo é tornar o processo simples e evitar armadilhas comuns que possam quebrar sua configuração.
...
Instale a versão estável mais recente do Bades.id com um único comando:
...
### Expor o Bades.id para Acesso Externo

Por padrão, o Bades.id é executado em `localhost` na porta `3000`. Para acessá-lo via um domínio externo ou endereço IP, você precisa configurar o `SERVER_URL` no seu arquivo `.env`.
...
Recomendamos fortemente configurar o Bades.id atrás de um proxy reverso com terminação SSL para segurança e funcionalidade ótimas.
