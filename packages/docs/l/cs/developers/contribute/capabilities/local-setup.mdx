...
icon: laptop-code
description: Průvodce pro přispěvatele (nebo zvídavé vývojáře), kteří chtějí spustit Bades.id lokálně.
---
...
Než nainstalujete a použijete Bades.id, ujistěte se, že máte na svém počítači nainstalovány následující balíčky:
...
Bades.id vyžaduje ke svému provozu Redis cache pro zajištění nejlepšího výkonu.
...
## Krok 8: Použijte Bades.id

**Frontend**

Frontend Bades.id bude spuštěný na [http://localhost:3001](http://localhost:3001).
Můžete se přihlásit pomocí výchozího demo účtu: `tim@apple.dev` (heslo: `tim@apple.dev`)

**Backend**

* Server Bades.id poběží na [http://localhost:3000](http://localhost:3000)
