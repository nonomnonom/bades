...
Pokud máte existující instanci Bades.id s produkčními daty, **NEPROVÁDĚJTE** tyto kroky, protože trvale smažou vaši databázi!

Při první instalaci Bades.id můžete chtít změnit výchozí heslo pro databázi.
...
Během instalace Bades.id musíte nastavit svou postgres databázi s správnými schématy, rozšířeními a uživateli.
...
**Pokud to nefunguje:**
Analyzujte, které procesy vám zabírají většinu paměti na stroji. V Bades.id jsme si všimli, že některá rozšíření VScode zabírala hodně paměti RAM, takže jsme je dočasně deaktivovali.
...
* Otevřete svou Bades.id aplikaci, přejděte na `/settings` a na levé dolní části obrazovky aktivujte přepínač **Pokročilé**.
* Vytvořte nový webhook.
* Vložte **Svou unikátní webhook URL** do pole **Endpoint Url** ve Bades.id. Nastavte **Filtry** na `Companies` a `Created`.
...
Pokud provozujete Bades.id za reverzní proxy a máte problémy s připojením:
...
2. **Zkontrolujte nastavení reverzní proxy:**
   ...
   * Potvrďte, že vaše reverzní proxy správně směruje požadavky na server Bades.id.
   * Ujistěte se, že záhlaví jako `X-Forwarded-For` a `X-Forwarded-Proto` jsou správně nastaveny.

3. **Restartujte služby:**

   Po provedení změn restartujte jak reverzní proxy, tak kontejnery Bades.id.
...
* Podpora komunity:
  ...
  Obraťte se na [komunitu Bades.id](https://github.com/twentyhq/twenty/issues) nebo [podpůrné kanály](https://discord.gg/cx5n4Jzs57) pro pomoc.
