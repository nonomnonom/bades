...
Tato příručka poskytuje podrobný návod k instalaci a konfiguraci aplikace Bades.id pomocí Docker Compose. Cílem je zjednodušit proces a zabránit běžným úskalím, která by mohla narušit vaše nastavení.
...
Nainstalujte nejnovější stabilní verzi Bades.id jedním příkazem:
...
### Otevření Bades.id pro Externí Přístup

Ve výchozím nastavení běží Bades.id na `localhost` na portu `3000`. Pro přístup přes externí doménu nebo IP adresu musíte nakonfigurovat `SERVER_URL` ve vašem `.env` souboru.
...
SSL (HTTPS) je potřebné pro správné fungování některých funkcí prohlížeče. I když tyto funkce mohou fungovat během místního vývoje (protože prohlížeče zacházejí s localhost jinak), správná konfigurace SSL je nutná při hostování Bades.id na běžné doméně.
...
Důrazně doporučujeme nastavit Bades.id za reverzní proxy se SSL ukončením pro optimální bezpečnost a funkčnost.
