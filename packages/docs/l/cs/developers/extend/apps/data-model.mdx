---
title: Datový model
description: Definujte objekty, pole, role a metadata aplikace pomocí Bades.id SDK.
icon: database
---
...
<Note>
**Základní pole jsou vytvořena automaticky.** Když definujete vlastní objekt, Bades.id automaticky přidá standardní pole
...
Pomocí `defineField()` přidejte pole k objektům, které nevlastníte — například ke standardním objektům Bades.id (Person, Company atd.). nebo k objektům z jiných aplikací. Na rozdíl od inline polí v `defineObject()` vyžadují samostatná pole `objectUniversalIdentifier` k určení, který objekt rozšiřují:
...
Relace propojují objekty. Ve Bades.id jsou relace vždy obousměrné — definujete obě strany a každá strana odkazuje na tu druhou.
...
Chcete-li vytvořit relaci s vestavěným objektem Bades.id (Person, Company atd.), použijte `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
