---
title: Architektura
description: Jak fungují aplikace Bades.id — sandboxing, životní cyklus a stavební bloky.
icon: sitemap
---

Aplikace Bades.id jsou balíčky TypeScriptu, které rozšiřují váš pracovní prostor o vlastní objekty, logiku, komponenty UI a funkce AI. Běží na platformě Bades.id s plnou izolací (sandboxingem) a řízením oprávnění.
...
| **Frontendová komponenta**    | Izolované React UI na stránce Bades.id            | [Frontendové komponenty](/l/cs/developers/extend/apps/front-components) |
...
* **Frontendové komponenty** běží ve Web Workerech s využitím Remote DOM — jsou oddělené od hlavní stránky, ale vykreslují nativní prvky DOM (nikoli iframy). Komunikují s Bades.id prostřednictvím hostitelského API pro předávání zpráv.
...
* **`yarn twenty dev`** — sleduje vaše zdrojové soubory a průběžně synchronizuje změny s připojeným serverem Bades.id. Typovaný klient API se při změně schématu automaticky znovu vygeneruje.
...
<CardGroup cols={2}>
  ...
  <Card title="Frontendové komponenty" icon="window-maximize" href="/l/cs/developers/extend/apps/front-components">
    Izolované komponenty Reactu v uživatelském rozhraní Bades.id.
  </Card>
