...
Logické funkce jsou serverové funkce v TypeScriptu, které běží na platformě Bades.id. Mohou být spouštěny požadavky HTTP, plány cronu nebo databázovými událostmi — a lze je také zpřístupnit jako nástroje pro agenty AI.
...
Logické funkce lze zpřístupnit jako **nástroje** pro agenty AI a pracovní postupy. Když je funkce označena jako nástroj, stane se dohledatelnou funkcemi AI produktu Bades.id a lze ji použít v automatizacích pracovních postupů.
...
* Proměnné prostředí `APPLICATION_ID`, `APP_ACCESS_TOKEN` a `API_URL` jsou dostupné uvnitř obslužné funkce (stejně jako u jakékoli jiné logické funkce), takže můžete volat Bades.id API s aplikačním přístupovým tokenem omezeným na vaši aplikaci.
...
Balíček `twenty-client-sdk` poskytuje dva typované klienty GraphQL pro práci s Bades.id API z vašich logických funkcí a frontendových komponent.
...
<Note>
  Když váš kód běží na Bades.id (logické funkce nebo frontendové komponenty), platforma vloží přihlašovací údaje jako proměnné prostředí:

  * `TWENTY_API_URL` — Základní URL Bades.id API
