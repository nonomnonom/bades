...
title: Publikování
icon: nahrát
description: Distribuujte svou aplikaci Bades.id do Marketplace nebo ji nasaďte interně.
---
...
* **Nasaďte tarball** — nahrajte svou aplikaci přímo na konkrétní server Bades.id pro interní nebo soukromé použití.
* **Publish to npm** — uveďte svou aplikaci v Marketplace Bades.id, aby ji mohl kterýkoli pracovní prostor objevit a nainstalovat.
...
U aplikací, které nechcete zpřístupnit veřejně — proprietární nástroje, integrace pouze pro enterprise nebo experimentální buildy — můžete nasadit tarball přímo na server Bades.id.
...
Aplikace vygenerované pomocí `create-bades-app` jsou hned připravené se dvěma workflowy GitHub Actions ve složce `.github/workflows/`. Jsou připravené ke spuštění hned, jakmile repozitář pushnete na GitHub — pro CI není potřeba žádné další nastavení a CD vyžaduje pouze jeden secret.
...
2. Spustí izolovanou testovací instanci Bades.id pomocí složené akce `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` (ekvivalent v CI k `yarn twenty server start --test`).
...
* `TWENTY_VERSION` (env, výchozí hodnota `latest`) — uzamkněte v CI používanou verzi serveru Bades.id úpravou této hodnoty v `ci.yml`.
...
Nasazuje vaši aplikaci na nakonfigurovaný server Bades.id při každém pushi do `main` a volitelně také z pull requestu, pokud je přidán štítek `deploy`.
...
| Nastavení               | Kde                                                             | Účel                                                                                                 |
| ----------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `env` v `cd.yml` (výchozí `http://localhost:3000`)              | Server Bades.id, na který se nasazuje. Před prvním použitím to změňte na skutečnou URL vašeho serveru. |
...
Publikování na npm zajistí, že bude vaše aplikace dohledatelná v Marketplace Bades.id. Jakýkoli pracovní prostor Bades.id může procházet, instalovat a aktualizovat aplikace z Marketplace přímo z UI.

### Požadavky

* Účet na [npm](https://www.npmjs.com)
* Klíčové slovo `twenty-app` ve vašem poli `keywords` v souboru `package.json` (přidejte je ručně — ve výchozím nastavení není zahrnuto v šabloně `create-bades-app`)
...
Server Bades.id synchronizuje svůj katalog tržiště z registru npm **každou hodinu**.
...
<Note>
Pokud vaše aplikace nedefinuje `aboutDescription` v `defineApplication()`, tržiště automaticky použije soubor `README.md` vašeho balíčku z npm jako obsah stránky O aplikaci. To znamená, že můžete spravovat jediný soubor README jak pro npm, tak pro tržiště Bades.id. Pokud chcete v tržišti jiný popis, explicitně nastavte `aboutDescription`.
</Note>
...
Přejděte na stránku **Nastavení > Aplikace** v Bades.id, kde lze procházet a instalovat jak aplikace z tržiště, tak aplikace nasazené jako tarball.
