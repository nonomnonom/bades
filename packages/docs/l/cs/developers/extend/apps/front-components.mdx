...
description: Vytvářejte komponenty Reactu, které se vykreslují uvnitř uživatelského rozhraní Bades.id se sandboxovou izolací.
icon: window-maximize
---

Frontendové komponenty jsou React komponenty, které se vykreslují přímo v uživatelském rozhraní Bades.id. Běží v **izolovaném Web Workeru** s využitím Remote DOM — váš kód je sandboxovaný, ale vykresluje se nativně na stránce, nikoli v iframu.

## Kde lze použít frontendové komponenty

Frontendové komponenty se mohou vykreslovat na dvou místech v rámci Bades.id:
...
Protože komponenta vrací `null`, Bades.id přeskočí vykreslení kontejneru — v rozvržení se neobjeví žádné prázdné místo. Komponenta má však stále přístup ke všem hookům a API komunikace s hostitelem.
...
* **Komponenty Bades.id UI** — import z `twenty-sdk/ui` (Button, Tag, Status, Chip, Avatar a další)
