...
* **Docker** — [Stáhnout zde](https://www.docker.com/products/docker-desktop/). Nutné pro spuštění lokální instance Bades.id. Není potřeba, pokud už máte spuštěný server Bades.id.
...
### Nastavte lokální instanci Bades.id

Generátor kostry se zeptá:

> **Chcete nastavit lokální instanci Bades.id?**

* **Zadejte `yes`** (doporučeno) — Stáhne image Dockeru `twenty-app-dev` a spustí lokální server Bades.id na portu `2020`. Než budete pokračovat, ujistěte se, že Docker běží.
* **Zadejte `no`** — Zvolte, pokud už máte lokálně spuštěný server Bades.id.
...
Poté se otevře okno prohlížeče se stránkou přihlášení do Bades.id. Přihlaste se předpřipraveným demo účtem:

* **E-mail:** `tim@apple.dev`
* **Heslo:** `tim@apple.dev`

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Přihlašovací obrazovka Bades.id" />
</div>
...
<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Autorizační obrazovka Bades.id CLI" />
</div>
...
Sleduje zdrojové soubory, při každé změně znovu sestaví a automaticky synchronizuje vaši aplikaci s lokálním serverem Bades.id. V terminálu byste měli vidět panel se stavem v reálném čase.
...
Oba režimy vyžadují server Bades.id běžící v režimu vývoje a autentizovaný remote — platí stejné požadavky.

### Zobrazte svou aplikaci v Bades.id
...
Klikněte na **My twenty app** a otevřete její **registraci aplikace**. Registrace je záznam na úrovni serveru, který popisuje vaši aplikaci — její název, jedinečný identifikátor, přihlašovací údaje OAuth a zdroj (lokální, npm nebo tarball). Existuje na serveru, ne uvnitř žádného konkrétního pracovního prostoru. Když nainstalujete aplikaci do pracovního prostoru, Bades.id vytvoří **aplikaci** v rozsahu pracovního prostoru, která odkazuje zpět na tuto registraci. Jedna registrace může být nainstalována ve více pracovních prostorech na stejném serveru.
...
| **Frontendové komponenty** | Komponenty Reactu, které se vykreslují v uživatelském rozhraní Bades.id (postranní panel, widgety, příkazová nabídka) |
...
Nástroj pro vytvoření kostry vám již spustil lokální server Bades.id. Pro jeho správu později použijte `yarn twenty server`:
...
Pokud dáváte přednost vlastnímu nastavení místo použití `create-bades-app`, můžete to udělat ve dvou krocích.
...
Pořád se nedaří? Požádejte o pomoc na [Discordu Bades.id](https://discord.com/channels/1130383047699738754/1130386664812982322).
