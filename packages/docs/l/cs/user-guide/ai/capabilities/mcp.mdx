---
title: MCP Server
description: Připojte asistenty AI ke svému pracovnímu prostoru Bades.id pomocí protokolu Model Context Protocol.
---
...
Bades.id zpřístupňuje server [MCP](https://modelcontextprotocol.io/), aby AI asistenti — Claude Desktop, Claude Code, Cursor, ChatGPT a další — mohli číst a zapisovat data vašeho CRM pomocí přirozeného jazyka.

Použijte svou **adresu URL pracovní oblasti** (adresu URL, kterou používáte k přístupu k Bades.id) jako koncový bod MCP. V Bades.id Cloud může vaše adresa URL pracovní oblasti být `https://{mycompany}.bades.id` nebo vlastní doména. Server je dostupný na:

| Prostředí           | Koncový bod MCP                                                               |
| ------------------- | ----------------------------------------------------------------------------- |
| **Cloud**           | `https://{your-workspace-url}/mcp` (např. `https://mycompany.bades.id/mcp`) |
...
Přidejte toto do konfigurace svého klienta MCP a nahraďte `{your-workspace-url}` hostitelem vaší pracovní oblasti (např. `mycompany.bades.id`):
...
1. Získá metadata OAuth služby Bades.id pomocí `/.well-known/oauth-protected-resource` a `/.well-known/oauth-authorization-server`
...
V Bades.id přejděte do **Settings > AI > More > MCP Server**. Vyberte metodu ověření (OAuth nebo klíč API), zkopírujte úryvek JSON (už bude používat adresu URL vaší pracovní oblasti) a vložte jej do konfiguračního souboru klienta MCP.
...
Restartujte svého klienta MCP (nebo znovu načtěte konfiguraci). Pokud používáte OAuth, budete přesměrováni do Bades.id k autorizaci přístupu. Pokud používáte klíč API, připojení je okamžité.
...
Po připojení server MCP zpřístupní nástroje, které odpovídají rozhraní Bades.id API. Doporučený pracovní postup je:
...
U samostatně hostovaných instancí nahraďte `{your-workspace-url}` adresou URL svého serveru. Ujistěte se, že `SERVER_URL` ve vašem prostředí odpovídá veřejné adrese URL vaší instance Bades.id — tato hodnota se používá k vygenerování metadat pro zjišťování OAuth.
