...
Jednotné přihlášení (SSO) umožňuje členům vašeho týmu přihlašovat se do Bades.id pomocí poskytovatele identity vaší organizace. To přináší:
...
* Administrátorský přístup k pracovnímu prostoru Bades.id

<Note>
**Pro uživatele vlastního hostování, kteří chtějí nastavit SSO**, obraťte se na contact@bades.id
</Note>
...
* Odeberte je z pracovního prostoru Bades.id
...
* **Mapování skupin**: Namapujte skupiny z IdP na role v Bades.id (pokud je k dispozici)
* **Proces offboardingu**: Zahrňte Bades.id do svého pracovního postupu deprovisioningu
