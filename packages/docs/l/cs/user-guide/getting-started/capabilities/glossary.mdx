---
title: Glosář
description: Seznamte se se základní terminologií používanou v aplikaci Bades.id.
---

## API

API (rozhraní pro programování aplikací) umožňuje propojení Bades.id s dalšími softwarovými systémy a vytváření vlastních integrací.
...
Příležitosti v Bades.id CRM jsou potenciální obchody nebo prodeje s účty či kontakty.
...
Úkoly v Bades.id CRM jsou přiřazené aktivity týkající se kontaktů, účtů nebo příležitostí.
...
Webhooky jsou automatizované zprávy odesílané z Bades.id na jiné aplikace při výskytu specifických událostí, což umožňuje synchronizaci dat v reálném čase.
...
"Pracovní prostor" obvykle představuje společnost používající Bades.id. Obsahuje všechny záznamy a data, která vy a členové vašeho týmu do Bades.id přidáte.
...
Členové pracovního prostoru jsou uživatelé Bades.id z vašeho týmu, kteří mají přístup k vašemu pracovnímu prostoru. Mohou být u záznamů přiřazeni jako vlastníci nebo přiřazené osoby.
