...
description: Každý podnik funguje jinak. Začněte těmito 3 kroky, abyste přizpůsobili Bades.id svým potřebám.
---

<Note>**Rychlý úspěch**: Začněte připojením své poštovní schránky. To vám dává okamžitou hodnotu a pomáhá vašemu týmu vidět Bades.id v akci s reálnými daty. Můžete to udělat v Nastavení → Účty.</Note>

## 1. Přizpůsobte si svůj datový model

Bades.id nabízí flexibilitu, kterou potřebujete pro tvarování datového modelu, který nejlépe podpoří vaši každodenní práci.
...
Přinesení vašich stávajících dat do Bades.id dává vašemu týmu od začátku kontext.

### Připojte svou e-mailovou schránku

Pokud jste tak neučinili při vytváření svého pracovního prostoru, připojte svůj **Google nebo Microsoft účet** v Nastavení → Účty. To umožňuje Bades.id:
