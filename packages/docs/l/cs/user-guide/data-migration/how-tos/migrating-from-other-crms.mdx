...
description: Podrobný průvodce krok za krokem pro migraci vašich dat z libovolného CRM do Bades.id.
---

## Přehled

Tento průvodce vás provede migrací vašich dat z libovolného CRM do Bades.id. Proces zahrnuje audit vašich dat, přípravu vašeho pracovního prostoru Bades.id, export z vašeho současného systému a import do Bades.id.
...
Vytvořte mapovací dokument mezi vaším současným CRM a Bades.id:
...
## Krok 3: Nastavte svůj pracovní prostor Bades.id

Před importem dat připravte svůj pracovní prostor Bades.id:
...
Pokud vaše data obsahují odkazy na uživatele (vlastník účtu, přiřazený uživatel apod.), tito uživatelé musí v Bades.id existovat před importem. Jinak tyto relace nelze namapovat.
</Warning>
...
Otevřete každý exportovaný soubor CSV v tabulkovém editoru a připravte jej pro Bades.id.

### Odstraňte duplicity

1. Seřaďte podle jedinečného pole (e-mail u lidí, doména u společností)
2. Odstraňte nebo sloučte duplicitní řádky
3. Ověřte, že v Bades.id již neexistují žádné duplicity
...
Použijte `https://domain.com` (ne `domain.com` ani `www.domain.com`). To odpovídá formátu Bades.id a předchází duplicitám, když připojíte synchronizaci e-mailu/kalendáře.
</Warning>
...
2. Použijte stejné e-mailové adresy, které uživatelé použili při připojení do vašeho pracovního prostoru Bades.id
...
1. Přejděte k objektu v Bades.id
...
* Bades.id začne synchronizovat e-maily k záznamům kontaktů
...
<AccordionGroup>
  <Accordion title="Mohu migrovat pracovní postupy automaticky?">
    V tuto chvíli ne. Pracovní postupy je nutné v Bades.id znovu vytvořit ručně.
  </Accordion>
...
Kontaktujte [contact@bades.id](mailto:contact@bades.id) nebo si prohlédněte naše [Implementační služby](/l/cs/user-guide/getting-started/capabilities/implementation-services).
