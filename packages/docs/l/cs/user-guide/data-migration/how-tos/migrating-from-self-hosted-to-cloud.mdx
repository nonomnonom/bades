...
description: Podrobný průvodce krok za krokem pro migraci vaší instance Bades.id hostované na vlastním serveru do Bades.id Cloud.
---

## Přehled

Tento průvodce vás provede migrací vašich dat z instance Bades.id hostované na vlastním serveru do Bades.id Cloud. Proces zahrnuje zřízení vašeho cloudového pracovního prostoru, export dat a jejich opětovný import.
...
1. Přejděte na [app.bades.id](https://app.bades.id) a vytvořte nový pracovní prostor
...
U složitých migrací nebo velkých datových sad nás kontaktujte na [contact@bades.id](mailto:contact@bades.id) nebo si prohlédněte naše [Implementační služby](/l/cs/user-guide/getting-started/capabilities/implementation-services).
