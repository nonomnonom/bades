...
Po nahrání souboru a namapování sloupců Bades.id vaše data ověří:

1. **Proběhne validace** — Bades.id zkontroluje každý řádek na chyby
...
| --------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| Duplicitní hodnoty zvýrazněné žlutě                                   | Hodnota již existuje v Bades.id nebo se ve vašem souboru vyskytuje dvakrát |
...
**Možnost 3: Nechte Bades.id aktualizovat existující záznam**
...
3. Bades.id aktualizuje existující záznam místo vytvoření duplicity
...
Pokud se shoduje alespoň jeden jedinečný identifikátor, Bades.id záznam aktualizuje.
</Note>
...
3. Zkontrolujte, zda záznamy v Bades.id již existují
...
To znamená, že v Bades.id neexistuje společnost s tímto konkrétním identifikátorem.
...
Pokoušíte se propojit se záznamem, který v Bades.id neexistuje.
...
Sloupec ve vašem souboru nelze namapovat, protože pole v Bades.id neexistuje.
...
1. **Uživatel nepřijal pozvánku** — uživatel zatím v Bades.id neexistuje
2. **Používáte uživatelské ID ze starého systému** — Bades.id nedokáže spárovat ID z jiného systému
3. **Nesprávný formát e-mailu** — e-mail neodpovídá účtu uživatele v Bades.id

### Jak to opravit

1. Ujistěte se, že všichni uživatelé **přijali svou pozvánku** do vašeho pracovního prostoru Bades.id
2. Použijte **e-mailovou adresu** uživatele (ne jeho jméno nebo ID ze starého systému)
3. Použijte stejný e-mail, jaký použili k připojení do Bades.id
...
<Check>Formát domény přesně odpovídá formátu Bades.id</Check>
...
1. **Stáhněte si ukázkový soubor** — uvidíte přesný formát, který Bades.id očekává
