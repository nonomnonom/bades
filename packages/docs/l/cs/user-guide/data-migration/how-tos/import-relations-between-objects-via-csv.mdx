...
Nadřazený („jedna“ strana) musí v Bades.id existovat, než na něj budete moci odkazovat.
...
1. Exportujte nadřazené záznamy z Bades.id
...
Použijte `https://domain.com` (ne pouze `domain.com`). To odpovídá způsobu, jak Bades.id ukládá domény společností, a předchází chybám při párování.
</Warning>
...
Bades.id automaticky propojí každý podřízený záznam s odpovídajícím nadřazeným.
...
| **Neexistující záznamy**     | Nadřazený záznam v Bades.id neexistuje            | Nejprve importujte nadřazené záznamy nebo zkontrolujte překlepy |
| **Rozdílná velikost písmen** | `Acme.com` v souboru, ale v Bades.id `acme.com`   | Zajistěte přesnou shodu velikosti písmen                        |
...
<AccordionGroup>
  <Accordion title="Co když můj nadřazený záznam nemá doménu ani e-mail?">
    Máte dvě možnosti:
    1. Použijte Bades.id `id` (exportujte nadřazené záznamy, abyste získali jejich ID)
