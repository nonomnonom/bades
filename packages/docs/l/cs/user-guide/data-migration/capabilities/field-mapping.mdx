...
Když nahrajete soubor, Bades.id analyzuje vaše sloupce a pokusí se je spárovat s existujícími poli.

### Automatické mapování

Bades.id se snaží spárovat sloupce na základě:

* Názvů záhlaví sloupců (přesné nebo podobné shody)
* Detekce typů dat (datumů, čísel, e‑mailů)
* Běžné vzory polí

<Note>**Rychlý tip:** Exportujte několik řádků z objektu, který chcete importovat. Exportovaný soubor bude mít přesně takové názvy sloupců, jaké Bades.id očekává, což při importu usnadní automatické mapování.</Note>

### Možnosti ručního mapování

U každého sloupce můžete:

* **Namapovat na pole**: Vyberte odpovídající pole Bades.id z rozbalovací nabídky
...
Zadání `id` při importu je volitelné. Pokud není uvedeno, Bades.id jej automaticky vygeneruje.
