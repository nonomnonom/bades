---
title: Bades.id 앱
description: Bades.id 맞춤설정을 코드로 구축하고 관리하세요.
---
...
앱을 사용하면 Bades.id 맞춤설정을 **코드로** 구축하고 관리할 수 있습니다. 모든 것을 UI에서 구성하는 대신, 데이터 모델과 로직 함수를 코드로 정의합니다 — 이를 통해 더 빠르게 구축·유지 관리하고 여러 워크스페이스에 배포할 수 있습니다.
...
* Node.js 24+ 및 Yarn 4
* Bades.id 워크스페이스와 API 키(https://app.bades.id/settings/api-webhooks에서 생성)
...
참고: [create-bades-app](https://www.npmjs.com/package/create-bades-app) 및 [twenty-sdk CLI](https://www.npmjs.com/package/twenty-sdk)의 CLI 참고 페이지도 확인하세요.

## 프로젝트 구조(스캐폴딩됨)

`npx create-bades-app@latest my-twenty-app`를 실행하면, 스캐폴더가 다음을 수행합니다:
...
* `yarn app:dev`는 `node_modules/twenty-sdk/generated`에 타입드 Bades.id 클라이언트를 자동으로 생성합니다.
...
<Note>
  **기본 필드는 자동으로 생성됩니다.** 사용자 정의 객체를 정의하면 Bades.id가 `name`, `createdAt`, `updatedAt`, `createdBy`, `position`, `deletedAt` 등의 표준 필드를 자동으로 추가합니다. 이 필드들은 `fields` 배열에 정의할 필요가 없습니다. 사용자 정의 필드만 추가하세요.
</Note>
...
`yarn app:dev`는 `node_modules/twenty-sdk/generated`에 타입드 Bades.id 클라이언트를 자동으로 생성합니다. 함수에서 사용하세요:
...
함수가 Bades.id에서 실행될 때, 플랫폼은 코드가 실행되기 전에 자격 증명을 환경 변수로 주입합니다:

* `TWENTY_API_URL`: 앱이 대상으로 하는 Bades.id API의 기본 URL.
...
최적의 시작 경험을 위해 `create-bades-app` 사용을 권장하지만, 프로젝트를 수동으로 설정할 수도 있습니다. CLI를 전역으로 설치하지 마세요. 대신 `twenty-sdk`를 로컬 종속성으로 추가하고 package.json에 스크립트를 연결하세요:
...
* 서버에 연결할 수 없음: API URL과 Bades.id 서버에 접근 가능한지 확인하세요.
