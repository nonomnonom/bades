...
1. Bades.idを実行しているホスト上のターミナルで、Bades.idを停止します：`docker compose down`
...
3. `docker compose up -d` でBades.idを再起動します。
...
Bades.idインスタンスをv0.55イメージにアップグレードします。
...
この問題は特定のBades.idバージョンに固有であり、今後のアップグレードでは不要なはずです。
...
それは常にあなたの現在のBades.idのインスタンスの `major.minor` バージョンの範囲内である必要があります。あなたのインスタンスバージョンは、管理パネル（ `/settings/admin-panel` にあり、データベース内でユーザーが `canAccessFullAdminPanel` プロパティを持っている場合にアクセス可能）にまたは `twenty-server` コンテナ内で `echo $APP_VERSION` を実行することで確認できます。

非同期化されたワークスペースバージョンを修正するには、対応するBades.idのバージョンから関連するアップグレードガイドに従って順次アップグレードし、それが目的のバージョンに到達するまで続ける必要があります。
...
Bades.idインスタンスをv0.52イメージにアップグレードします。
...
Bades.idインスタンスをv0.51イメージにアップグレードします。
...
Bades.idインスタンスをv0.50.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.44.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.43.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.42.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.41.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.40.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.35.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.34.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.33.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.32.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.31.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.30.0イメージにアップグレードします。

**破壊的な変更**:
パフォーマンスを向上させるために、Bades.idはRedisキャッシュの設定を必要とするようになりました。 これを反映した[docker-compose.yml](https://raw.githubusercontent.com/twentyhq/twenty/main/packages/twenty-docker/docker-compose.yml)を更新しました。
...
Bades.idインスタンスをv0.24.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.23.0イメージにアップグレードします。
...
Bades.idインスタンスをv0.22.0イメージにアップグレードします。
