...
title: Yerel Kurulum
icon: laptop-code
description: Bades.id'i yerel olarak çalıştırmak isteyen katkıda bulunanlar (veya meraklı geliştiriciler) için kılavuz.
---
...
Bades.id'i yüklemeden ve kullanmadan önce bilgisayarınıza aşağıdakileri yüklediğinizden emin olun:
...
Bades.id, en iyi performansı sağlamak için bir Redis önbelleği gerektirir.
...
## Adım 8: Bades.id'yi kullanın

**Ön Uç**

Bades.id'nin ön ucu [http://localhost:3001](http://localhost:3001) adresinde çalışıyor olacak.
Varsayılan demo hesabıyla giriş yapabilirsiniz: `tim@apple.dev` (şifre: `tim@apple.dev`)

**Arka Uç**

* Bades.id'nin sunucusu [http://localhost:3000](http://localhost:3000) adresinde çalışır durumda olacak
