...
| UI uzantılarına sahip Bades.id Uygulaması             | [Uygulamalar](/l/tr/developers/extend/apps/getting-started) (OAuth otomatik olarak yönetilir) |

## Bir İstemci Kaydedin

Bades.id, [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591) uyarınca **dinamik istemci kaydını** destekler. Manuel kurulum gerekmez — programatik olarak kaydedin:
...
Uygulamanız bir Bades.id kullanıcısı adına hareket ettiğinde bu akışı kullanın.
...
Yetkilendirmeden sonra, Bades.id `redirect_uri` adresinize geri yönlendirir:
...
Bades.id, OAuth yapılandırmasını standart bir keşif uç noktasında yayımlar:
