---
title: Bades.id Uygulamaları
description: Bades.id özelleştirmelerini kod olarak oluşturun ve yönetin.
---
...
* **Ön yüz bileşenleri** — Bades.id'nin kullanıcı arayüzünde (kayıt sayfaları, komut menüsü, yan paneller) görüntülenen React bileşenleri
* **Yapay zekâ yetenekleri ve ajanları** — Bades.id'nin yapay zekâsını özel yeteneklerle genişletin
...
Bu, yeni bir uygulamanın iskeletini oluşturur, isteğe bağlı olarak yerel bir Bades.id sunucusunu başlatır ve dosyalarınızdaki değişiklikleri izlemeye başlar. Tam adım adım anlatım için [Başlarken](/l/tr/developers/extend/apps/getting-started) kılavuzuna bakın.
...
| `defineFrontComponent()`           | Bades.id'nin kullanıcı arayüzündeki React bileşenleri        |
...
3. **`yarn twenty deploy`** — uzak bir Bades.id sunucusuna dağıtır
...
yarn twenty remote add    # Bir Bades.id sunucusuna bağlan
