...
* `serverVariables` kullanın (`applicationVariables` değil) — OAuth kimlik bilgileri sunucu genelidir ve her Bades.id sunucusu için bir OAuth uygulaması vardır.
...
4. Yüklü uygulamayı Bades.id'de bir sunucu yöneticisi olarak açın → karşılık gelen `serverVariables` üzerinde değerleri ayarlayın.
