---
title: Mimari
description: Bades.id uygulamaları nasıl çalışır — korumalı alan, yaşam döngüsü ve yapı taşları.
icon: sitemap
---

Bades.id uygulamaları, çalışma alanınızı özel nesneler, mantık, UI bileşenleri ve yapay zekâ yetenekleriyle genişleten TypeScript paketleridir. Tam korumalı alan ve izin kontrolleriyle Bades.id platformunda çalışırlar.
...
| **Ön Uç Bileşeni**       | Bades.id'nin sayfasında korumalı alanda React kullanıcı arayüzü | [Ön Uç Bileşenleri](/l/tr/developers/extend/apps/front-components)         |
...
* **Ön uç bileşenleri**, Remote DOM kullanan Web Worker'larda çalışır — ana sayfadan yalıtılmıştır ancak yerel DOM öğelerini (iframe'ler değil) oluşturur. Bades.id ile mesaj iletimi yapan bir ana makine API'si aracılığıyla iletişim kurarlar.
...
* **`yarn twenty dev`** — kaynak dosyalarınızı izler ve bağlı bir Bades.id sunucusuna değişiklikleri canlı olarak senkronize eder. Şema değiştiğinde tipli API istemcisi otomatik olarak yeniden oluşturulur.
...
<CardGroup cols={2}>
  ...
  <Card title="Ön uç bileşenleri" icon="window-maximize" href="/l/tr/developers/extend/apps/front-components">
    Bades.id'nin kullanıcı arayüzünde korumalı alanda React bileşenleri.
  </Card>
