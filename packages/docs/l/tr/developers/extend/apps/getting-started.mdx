...
title: Başlarken
icon: rocket
description: İlk Bades.id uygulamanızı dakikalar içinde oluşturun.
---
...
* **Docker** — [Buradan indirin](https://www.docker.com/products/docker-desktop/). Yerel bir Bades.id sunucusunu çalıştırmak için gereklidir. Zaten başka bir yerde Bades.id çalıştırıyorsanız atlayın.

Bir Bades.id uygulaması oluşturmanın üç aşaması vardır. İskelet oluşturucu bunları tek bir sorunsuz akış komutuna indirger, ancak her aşama ayrı bir kavramdır — bir şeyler başarısız olduğunda hangi aşamada olduğunuzu bilmek neyi düzeltmeniz gerektiğini söyler.

| Aşama                        | Ne yaparsınız                                     | Araç                          | Sonuç                              |
| ---------------------------- | ------------------------------------------------- | ----------------------------- | ---------------------------------- |
| **1. İskelet Oluşturma**     | Uygulamanın kaynak kodunu oluşturun               | `npx create-bades-app`       | Diskte bir TypeScript projesi      |
| **2. Bir sunucu çalıştırma** | Eşitleme yapacağınız bir Bades.id sunucusu başlatın | Docker + `yarn twenty server` | Çalışan bir Bades.id örneği          |
...
## Aşama 2 — Yerel bir Bades.id sunucusu çalıştırın

Uygulamanızın eşitleme yapacağı bir Bades.id sunucusuna ihtiyacı vardır. Sunucu, Docker içinde yerel olarak çalışan tam bir Bades.id örneğidir — UI, GraphQL API, PostgreSQL. Yerel kodunuz tanımlarını bu sunucuya yükler; bu da onların arayüzde görünmesini sağlar.

İskelet oluşturucu sizin için bir tane başlatmayı teklif eder:

> **Yerel bir Bades.id örneği kurmak ister misiniz?**
...
* **Hayır** — Zaten bağlanmak istediğiniz bir Bades.id sunucunuz varsa bunu seçin. Bunu daha sonra `yarn twenty remote add` ile bağlayabilirsiniz.
...
<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Bades.id oturum açma ekranı" />
</div>

Sonraki ekranda **Authorize**'a tıklayın — bu işlem CLI'nin çalışma alanınıza erişmesine izin verir.

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Bades.id CLI yetkilendirme ekranı" />
</div>
...
**Bu aşamadan sonra:** [http://localhost:2020](http://localhost:2020) adresinde çalışan bir Bades.id sunucunuz ve ona eşitleme yapmak için yetkilendirilmiş bir CLI'ınız olur.
...
<Warning>
Geliştirme modu yalnızca geliştirme ortamında (`NODE_ENV=development`) çalışan Bades.id örneklerinde kullanılabilir. Üretim örnekleri, geliştirme eşitleme isteklerini reddeder — üretim sunucularına dağıtmak için `yarn twenty deploy` kullanın. Bkz. [Uygulamaları Yayımlama](/l/tr/developers/extend/apps/publishing).
</Warning>
...
| **Ön uç bileşenleri**     | Bades.id'nin arayüzünde görüntülenen React bileşenleri (yan panel, widget'lar, komut menüsü)                |
...
Yerel Bades.id konteynerini kontrol etmek için `yarn twenty server` kullanın:
...
Takıldınız mı? [Bades.id Discord](https://discord.com/channels/1130383047699738754/1130386664812982322) üzerinde yardım isteyin.
