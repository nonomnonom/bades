...
description: Bades.id'nin UI'si içinde, korumalı alan (sandbox) izolasyonu ile görüntülenen React bileşenleri oluşturun.
icon: window-maximize
---

Ön uç bileşenler, Bades.id'nin UI'si içinde doğrudan görüntülenen React bileşenleridir. Remote DOM kullanan izole bir Web Worker içinde çalışırlar — kodunuz izole bir ortamda (sandbox) çalışır ancak bir iframe içinde değil, sayfada yerel olarak işlenir.

## Ön uç bileşenlerinin kullanılabileceği yerler

Ön uç bileşenler, Bades.id içinde iki konumda işlenebilir:
...
Bileşen `null` döndürdüğü için, Bades.id bunun için bir kapsayıcı oluşturmayı atlar — düzende boş alan görünmez. Bileşen yine de tüm hook'lara ve host iletişim API'sine erişime sahiptir.
...
| `useSelectedRecordIds()`                      | `metin[]`            | Tüm seçili kayıt kimlikleri (hiçbiri seçilmediyse boş dizi)                |
...
* **Bades.id UI bileşenleri** — `twenty-sdk/ui` içinden içe aktarın (Button, Tag, Status, Chip, Avatar ve daha fazlası)
