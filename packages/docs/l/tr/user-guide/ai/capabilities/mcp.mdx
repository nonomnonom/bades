---
title: MCP Sunucusu
description: Model Context Protocol kullanarak yapay zeka asistanlarını Bades.id çalışma alanınıza bağlayın.
---
...
Bades.id, yapay zeka asistanlarının — Claude Desktop, Claude Code, Cursor, ChatGPT ve diğerlerinin — doğal dil aracılığıyla CRM verilerinizi okuması ve yazması için bir [MCP](https://modelcontextprotocol.io/) sunucusu sunar.

MCP uç noktası olarak **çalışma alanı URL'nizi** (Bades.id'ye erişmek için kullandığınız URL) kullanın. Bades.id Cloud'da, çalışma alanı URL'niz `https://{mycompany}.bades.id` ya da özel bir alan adı olabilir. Sunucu şu adreste kullanılabilir:

| Ortam                        | MCP Uç Noktası                                                               |
| ---------------------------- | ---------------------------------------------------------------------------- |
| **Bulut**                    | `https://{your-workspace-url}/mcp` (örn. `https://mycompany.bades.id/mcp`) |
...
Bunu MCP istemci yapılandırmanıza ekleyin; `{your-workspace-url}` öğesini çalışma alanınızın ana makine adıyla değiştirin (örn. `mycompany.bades.id`):
...
1. Bades.id'nin OAuth üst verilerini `/.well-known/oauth-protected-resource` ve `/.well-known/oauth-authorization-server` üzerinden keşfeder
...
Bades.id içinde **Settings > AI > More > MCP Server** bölümüne gidin. Kimlik doğrulama yöntemini (OAuth veya API Anahtarı) seçin, JSON parçacığını kopyalayın (çalışma alanı URL'nizi zaten kullanacaktır) ve MCP istemcinizin yapılandırma dosyasına yapıştırın.
...
MCP istemcinizi yeniden başlatın (veya yapılandırmayı yeniden yükleyin). OAuth kullanıyorsanız, erişimi yetkilendirmek için Bades.id'ye yönlendirileceksiniz. API anahtarı kullanıyorsanız, bağlantı anında kurulur.
...
Bağlandıktan sonra, MCP sunucusu Bades.id API'sini yansıtan araçlar sunar. Önerilen iş akışı:
...
Öz barındırmalı kurulumlarda `{your-workspace-url}` değerini sunucunuzun URL'siyle değiştirin. Ortamınızdaki `SERVER_URL` değerinin Bades.id örneğinizin genel URL'siyle eşleştiğinden emin olun — bu, OAuth keşif üst verilerini oluşturmak için kullanılır.
