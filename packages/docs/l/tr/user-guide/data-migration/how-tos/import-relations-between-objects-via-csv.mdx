...
Ana ("bir" taraf) kaydı, ona referans verebilmeden önce Bades.id'de mevcut olmalıdır.
...
1. Ana kayıtları Bades.id'den dışa aktarın
...
`https://domain.com` kullanın (yalnızca `domain.com` değil). Bu, Bades.id'nin Şirket alan adlarını saklama biçimiyle eşleşir ve eşleştirme hatalarını önler.
</Warning>
...
Bades.id, her alt kaydı otomatik olarak eşleşen ana kayda bağlar.
...
| **Mevcut olmayan kayıtlar**      | Ana kayıt Bades.id'de mevcut değil               | Önce ana kayıtları içe aktarın veya yazım hatalarını kontrol edin |
| **Büyük/küçük harf uyumsuzluğu** | Dosyada `Acme.com`, Bades.id'de ise `acme.com`   | Bire bir büyük/küçük harf eşleşmesini sağlayın                    |
...
<AccordionGroup>
  <Accordion title="Ana kaydımın alan adı veya e-postası yoksa ne olur?">
    İki seçeneğiniz var:
    1. Bades.id `id` değerini kullanın (kimlikleri almak için ana kayıtları dışa aktarın)
