...
description: Verilerinizi herhangi bir CRM'den Bades.id'ye taşımak için adım adım kılavuz.
---

## Genel Bakış

Bu kılavuz, verilerinizi herhangi bir CRM'den Bades.id'ye taşıma sürecinde size yol gösterir. Süreç, verilerinizi denetlemeyi, Bades.id çalışma alanınızı hazırlamayı, mevcut sisteminizden dışa aktarmayı ve Bades.id'ye içe aktarmayı içerir.
...
Mevcut CRM'iniz ile Bades.id arasında bir eşleme belgesi oluşturun:
...
## Adım 3: Bades.id Çalışma Alanınızı Kurun

Verileri içe aktarmadan önce Bades.id çalışma alanınızı hazırlayın:
...
Verileriniz kullanıcı referansları içeriyorsa (Hesap Sahibi, Atanan vb.), bu kullanıcıların içe aktarmadan önce Bades.id'de mevcut olması gerekir. Aksi halde bu ilişkiler eşlenemez.
</Warning>
...
Dışa aktarılan her CSV'yi bir hesap tablosu uygulamasında açın ve Bades.id için hazırlayın.
...
3. Bades.id'de halihazırda yinelenen olmadığını doğrulayın
...
`https://domain.com` kullanın (`domain.com` veya `www.domain.com` değil). Bu, Bades.id'nin biçimiyle eşleşir ve e-posta/takvim eşzamanlamasını bağladığınızda yinelenmeleri önler.
</Warning>
...
2. Kullanıcıların Bades.id çalışma alanınıza katılmak için kullandıklarıyla aynı e-posta adreslerini kullanın
...
## Adım 6: Bades.id'ye İçe Aktarın
...
* Bades.id, e-postaları kişi kayıtlarıyla eşzamanlamaya başlayacaktır
...
<AccordionGroup>
  <Accordion title="İş akışlarını otomatik olarak taşıyabilir miyim?">
    Şu anda mümkün değil. İş akışları Bades.id'de manuel olarak yeniden oluşturulmalıdır.
  </Accordion>
...
[contact@bades.id](mailto:contact@bades.id) adresinden bizimle iletişime geçin veya [Uygulama Hizmetlerimizi](/l/tr/user-guide/getting-started/capabilities/implementation-services) keşfedin.
