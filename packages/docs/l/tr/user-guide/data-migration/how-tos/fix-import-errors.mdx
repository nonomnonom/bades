...
Dosyanızı yükledikten ve sütunları eşledikten sonra, Bades.id verilerinizi doğrular:

1. **Doğrulama çalışır** — Bades.id her satırı hatalar için kontrol eder
...
| ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| Yinelenen değerler sarı renkle vurgulanır                                      | Değer Bades.id'de zaten mevcut veya dosyanızda iki kez görünüyor |
...
**Seçenek 3: Mevcut kaydı Bades.id'nin güncellemesine izin verin**
...
3. Bades.id, yinelenen bir kayıt oluşturmak yerine mevcut kaydı güncelleyecektir
...
Benzersiz tanımlayıcılardan biri eşleştiği sürece Bades.id kaydı günceller.
</Note>
...
3. Kayıtların Bades.id'de zaten mevcut olup olmadığını kontrol edin
...
Bu, Bades.id'de o özel tanımlayıcıya sahip bir Şirket olmadığı anlamına gelir.
...
Bades.id'de mevcut olmayan bir kayda bağlanmaya çalışıyorsunuz.
...
Dosyanızdaki bir sütun eşlenemiyor çünkü alan Bades.id'de mevcut değil.
...
1. **Kullanıcı davetini kabul etmedi** — kullanıcı henüz Bades.id'de mevcut değil
2. **Eski sistemdeki kullanıcı kimliğini kullanma** — Bades.id başka bir sistemin kimliklerini eşleştiremez
3. **Yanlış e-posta biçimi** — e-posta, kullanıcının Bades.id hesabıyla eşleşmiyor

### Nasıl Düzeltilir

1. Tüm kullanıcıların Bades.id çalışma alanınıza **davetlerini kabul ettiğinden** emin olun
2. Kullanıcının **e-posta adresini** kullanın (adlarını veya eski sistem kimliğini değil)
3. Bades.id'ye katılmak için kullandıkları aynı e-postayı kullanın
...
<Check>Alan adı biçimi, Bades.id'nin biçimiyle birebir eşleşir</Check>
...
1. **Örnek dosyayı indirin** — Bades.id'nin beklediği tam biçimi görün
