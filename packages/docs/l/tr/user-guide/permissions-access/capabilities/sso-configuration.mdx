...
Tek Oturum Açma (SSO), ekip üyelerinizin kuruluşunuzun kimlik sağlayıcısını kullanarak Bades.id'ye giriş yapmasına olanak tanır. Bu şunları sağlar:
...
Bades.id, SSO'yu şu sağlayıcılarla destekler:
...
* Bades.id çalışma alanına yönetici erişimi

<Note>
**SSO'yu kurmak isteyen öz barındırma kullanıcıları için**, contact@bades.id ile iletişime geçin
</Note>
...
* Bades.id çalışma alanından kaldırın
...
* **Grup eşleme**: IdP gruplarını Bades.id rollerine eşleyin (varsa)
* **Ayrılış süreci**: Erişim kaldırma iş akışınıza Bades.id'yi dahil edin
