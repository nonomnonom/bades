...
export const MyComponent = () => {
  return (
      <Chip 
      size="large"
      label="Прозрачный неактивный чип"
      clickable={false}
      variant="rounded"
...
export const MyComponent = () => {
  return (
      <Chip 
      size="large"
      label="Неактивный чип, который показывает подсказку при переполнении."
      clickable={false}
      variant="regular"
      accent="text-primary"
      leftComponent
...
import { IconBades.idStar } from 'twenty-ui/display';
import { Chip } from 'twenty-ui/components';
