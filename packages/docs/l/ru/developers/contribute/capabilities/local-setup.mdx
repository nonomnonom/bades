...
icon: laptop-code
description: Руководство для участников (или любопытных разработчиков), которые хотят запускать Bades.id локально.
---
...
Прежде чем установить и использовать Bades.id, убедитесь, что у вас установлено следующее:
...
Bades.id требует кэша Redis для обеспечения наилучшей производительности.
...
## Шаг 8: Использовать Bades.id

**Frontend**

Фронтенд Bades.id будет работать на [http://localhost:3001](http://localhost:3001).
Вы можете войти, используя учетную запись демо по умолчанию: `tim@apple.dev` (пароль: `tim@apple.dev`)

**Backend**

* Сервер Bades.id будет работать на [http://localhost:3000](http://localhost:3000).
