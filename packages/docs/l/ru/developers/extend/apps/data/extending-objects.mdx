...
description: Добавляйте поля к стандартным объектам Bades.id (Person, Company, …) или к объектам из других приложений с помощью `defineField`.
icon: wand-magic-sparkles
---

Используйте `defineField()` для добавления поля к объекту, которым вы не владеете — стандартному объекту Bades.id, такому как Person или Company, или объекту, поставляемому другим установленным приложением. В отличие от встроенных полей, объявленных внутри [`defineObject`](/l/ru/developers/extend/apps/data/objects), отдельные поля требуют `objectUniversalIdentifier`, чтобы указать, какой объект они расширяют.
...
* `objectUniversalIdentifier` определяет целевой объект. Для стандартных объектов Bades.id импортируйте константу из `twenty-sdk`:
