---
title: Модель данных
description: Определяйте объекты, поля, роли и метаданные приложения с помощью Bades.id SDK.
icon: database
---
...
<Note>
**Базовые поля создаются автоматически.** Когда вы определяете пользовательский объект, Bades.id автоматически добавляет стандартные поля,
...
Используйте `defineField()` для добавления полей к объектам, которые вам не принадлежат — например, к стандартным объектам Bades.id (Person, Company и т. д.). или к объектам из других приложений. В отличие от встроенных полей в `defineObject()`, отдельные поля требуют `objectUniversalIdentifier`, чтобы указать, какой объект они расширяют:
...
Отношения связывают объекты между собой. В Bades.id отношения всегда двунаправленные — вы определяете обе стороны, и каждая сторона ссылается на другую.
...
Чтобы создать отношение со встроенным объектом Bades.id (Person, Company и т. д.), используйте `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
