...
Bades.id отправляет запрос HTTP POST на ваш URL каждый раз, когда запись создаётся, обновляется или удаляется. Поддерживаются все типы объектов, включая пользовательские объекты.
...
Bades.id отправляет вебхуки для следующих типов событий:
...
Bades.id подписывает каждый запрос вебхука для обеспечения безопасности. Проверяйте подписи, чтобы убедиться в подлинности запросов.

### Заголовки

| Заголовок                    | Описание              |
| ---------------------------- | --------------------- |
| `X-Bades.id-Webhook-Signature` | Подпись HMAC SHA256   |
| `X-Bades.id-Webhook-Timestamp` | Метка времени запроса |

### Шаги проверки

1. Получите метку времени из `X-Bades.id-Webhook-Timestamp`
2. Создайте строку: `{timestamp}:{JSON payload}`
3. Вычислите HMAC SHA256, используя секрет вашего вебхука
4. Сравните с `X-Bades.id-Webhook-Signature`
...
| **Триггер вебхука в рабочем процессе** | IN          | Получать данные в Bades.id из внешних систем                                 |
