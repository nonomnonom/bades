...
Objekte sind Strukturen, die es Ihnen ermöglichen, Daten (Aufzeichnungen, Attribute und Werte) zu speichern, die spezifisch für eine Organisation sind. Bades.id bietet sowohl Standard- als auch benutzerdefinierte Objekte.

Standardobjekte sind systemeigene Objekte mit einer Reihe von Attributen, die allen Benutzern zur Verfügung stehen. Beispiele für Standardobjekte in Bades.id sind Unternehmen und Person. Standardobjekte verfügen über Standardfelder, die ebenfalls allen Bades.id-Nutzern zur Verfügung stehen, wie z.B. Unternehmen.displayName.
