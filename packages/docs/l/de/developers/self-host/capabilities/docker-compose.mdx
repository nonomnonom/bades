...
Diese Anleitung enthält Schritt-für-Schritt-Anweisungen, um die Bades.id-Anwendung mit Docker Compose zu installieren und zu konfigurieren. Das Ziel ist, den Prozess übersichtlich zu gestalten und häufige Fallstricke zu vermeiden, die Ihre Einrichtung unterbrechen könnten.
...
Installieren Sie die neueste stabile Version von Bades.id mit einem einzigen Befehl:
...
### Bades.id für externen Zugriff freigeben

Standardmäßig läuft Bades.id auf `localhost` an Port `3000`. Um über eine externe Domain oder IP-Adresse darauf zuzugreifen, müssen Sie die `SERVER_URL` in Ihrer `.env`-Datei konfigurieren.
...
SSL (HTTPS) ist erforderlich, damit bestimmte Browserfunktionen ordnungsgemäß funktionieren. Obwohl diese Funktionen möglicherweise während der lokalen Entwicklung funktionieren (da Browser localhost anders behandeln), ist eine ordnungsgemäße SSL-Einrichtung erforderlich, wenn Bades.id auf einer regulären Domain gehostet wird.
...
Wir empfehlen dringend, Bades.id hinter einem Reverse-Proxy mit SSL-Beendigung für optimale Sicherheit und Funktionalität zu konfigurieren.
