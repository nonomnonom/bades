---
title: Bades.id-Apps
description: Bades.id-Anpassungen als Code erstellen und verwalten.
---
...
* **Frontend-Komponenten** — React-Komponenten, die innerhalb der UI von Bades.id gerendert werden (Datensatzseiten, Befehlsmenü, Seitenpanels)
* **KI-Fähigkeiten und -Agenten** — erweitern Sie die KI von Bades.id mit benutzerdefinierten Fähigkeiten
...
Dies erstellt das Grundgerüst für eine neue App, startet optional einen lokalen Bades.id-Server und beginnt, Ihre Dateien auf Änderungen zu überwachen. Den vollständigen Ablauf finden Sie im Leitfaden [Erste Schritte](/l/de/developers/extend/apps/getting-started).
...
| `defineFrontComponent()`           | React-Komponenten in der UI von Bades.id           |
...
3. **`yarn twenty deploy`** — stellt auf einem entfernten Bades.id-Server bereit
...
yarn twenty remote add    # Mit einem Bades.id-Server verbinden
