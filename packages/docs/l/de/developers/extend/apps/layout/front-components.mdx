...
description: Erstellen Sie React-Komponenten, die innerhalb der Bades.id-UI gerendert werden und durch eine Sandbox isoliert sind.
icon: window-maximize
---

Front-Komponenten sind React-Komponenten, die direkt innerhalb der Bades.id-UI gerendert werden. Sie laufen in einem **isolierten Web Worker** unter Verwendung von Remote DOM — Ihr Code wird in einer Sandbox ausgeführt, rendert jedoch nativ auf der Seite, nicht in einem iframe.

## Wo Front-Komponenten verwendet werden können

Front-Komponenten können an zwei Stellen innerhalb von Bades.id gerendert werden:
...
const HelloWorld = () => {
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <p>This component renders inside Bades.id.</p>
    </div>
  );
};
...
Da die Komponente `null` zurückgibt, überspringt Bades.id das Rendern eines Containers dafür — im Layout entsteht kein Leerraum. Die Komponente hat dennoch Zugriff auf alle Hooks und die Host-Kommunikations-API.
...
* **Bades.id-UI-Komponenten** — Import aus `twenty-sdk/ui` (Button, Tag, Status, Chip, Avatar und mehr)
