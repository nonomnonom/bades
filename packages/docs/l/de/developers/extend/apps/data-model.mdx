---
title: Datenmodell
description: Definieren Sie Objekte, Felder, Rollen und Anwendungsmetadaten mit dem Bades.id SDK.
icon: database
---
...
<Note>
**Basisfelder werden automatisch erstellt.** Wenn Sie ein benutzerdefiniertes Objekt definieren, fügt Bades.id automatisch Standardfelder hinzu
...
Verwenden Sie `defineField()`, um Objekten, die Ihnen nicht gehören — etwa Standardobjekten von Bades.id (Person, Company usw.) — Felder hinzuzufügen oder Objekten aus anderen Apps. Im Gegensatz zu Inline-Feldern in `defineObject()` benötigen eigenständige Felder einen `objectUniversalIdentifier`, um anzugeben, welches Objekt sie erweitern:
...
Relationen verbinden Objekte miteinander. In Bades.id sind Relationen stets **bidirektional** — Sie definieren beide Seiten, und jede Seite referenziert die andere.
...
Um eine Relation mit einem integrierten Bades.id-Objekt (Person, Company usw.) zu erstellen, verwenden Sie `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
