...
icon: hochladen
description: Veröffentlichen Sie Ihre Bades.id-App auf dem Bades.id-Marktplatz oder stellen Sie sie intern bereit.
---
...
* **Einen Tarball bereitstellen** — Laden Sie Ihre App direkt auf einen bestimmten Bades.id-Server für die interne oder private Nutzung hoch.
* **Auf npm veröffentlichen** — führen Sie Ihre App im Bades.id-Marktplatz auf, damit jeder Arbeitsbereich sie entdecken und installieren kann.
...
Für Apps, die Sie nicht öffentlich verfügbar machen möchten — proprietäre Tools, ausschließlich für Unternehmen bestimmte Integrationen oder experimentelle Builds — können Sie einen Tarball direkt auf einem Bades.id-Server bereitstellen.
...
Wenn Ihre App eine Funktion verwendet, die in einer bestimmten Bades.id-Serverversion eingeführt wurde (z. B. OAuth-Anbieter, die in v2.3.0 hinzugefügt wurden), sollten Sie die minimale Serverversion, die Ihre App benötigt, mithilfe des Felds `engines.twenty` in `package.json` angeben:
...
Apps, die mit `create-bades-app` erzeugt wurden, enthalten von Haus aus zwei GitHub-Actions-Workflows unter `.github/workflows/`. Sie sind einsatzbereit, sobald Sie das Repository zu GitHub pushen — für CI ist keine zusätzliche Einrichtung erforderlich, und für CD ist nur ein einziges Secret nötig.
...
2. Startet eine isolierte Bades.id-Testinstanz mithilfe der Composite-Action `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` (das CI-Äquivalent zu `yarn twenty docker:start --test`).
...
* `TWENTY_VERSION` (env, standardmäßig `latest`) — fixieren Sie die in CI verwendete Bades.id-Server-Version, indem Sie dies in `ci.yml` anpassen.
...
Stellt Ihre App bei jedem Push auf `main` auf einem konfigurierten Bades.id-Server bereit und optional aus einem Pull Request, wenn das Label `deploy` gesetzt ist.
...
| ----------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `env` in `cd.yml` (standardmäßig `http://localhost:3000`)        | Der Bades.id-Server, auf den bereitgestellt werden soll. Ändern Sie dies vor der ersten Verwendung auf die echte Server-URL. |
...
Die Veröffentlichung auf npm macht Ihre App im Bades.id-Marktplatz auffindbar. Jeder Bades.id-Arbeitsbereich kann Marktplatz-Apps direkt über die Benutzeroberfläche durchsuchen, installieren und aktualisieren.

### Anforderungen

* Ein [npm](https://www.npmjs.com)-Konto
* Das Schlüsselwort `twenty-app` in Ihrem `package.json`-Array `keywords` (manuell hinzufügen — es ist in der `create-bades-app`-Vorlage standardmäßig nicht enthalten)
...
Der Bades.id-Server synchronisiert seinen Marktplatzkatalog **stündlich** aus der npm-Registry.
...
<Note>
Wenn Ihre App keine `aboutDescription` in `defineApplication()` definiert, verwendet der Marktplatz automatisch die `README.md` Ihres Pakets von npm als Inhalt der Über-uns-Seite. Das bedeutet, dass Sie eine einzige README sowohl für npm als auch für den Bades.id-Marktplatz pflegen können. Wenn Sie im Marktplatz eine andere Beschreibung möchten, setzen Sie `aboutDescription` explizit.
</Note>
...
Gehen Sie zur Seite **Einstellungen > Anwendungen** in Bades.id, auf der sowohl Marktplatz- als auch per Tarball bereitgestellte Apps durchsucht und installiert werden können.
