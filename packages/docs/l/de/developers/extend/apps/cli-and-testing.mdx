...
Stellen Sie sicher, dass Ihr lokaler Bades.id-Server läuft, und führen Sie dann Folgendes aus:
...
<Note>
Dies unterscheidet sich von `yarn twenty server logs`, das die Docker-Container-Logs anzeigt. `yarn twenty logs` zeigt die Funktionsausführungsprotokolle Ihrer App vom Bades.id-Server.
</Note>
...
Ein **Remote** ist ein Bades.id-Server, mit dem sich Ihre App verbindet. Während der Einrichtung erstellt das Scaffolding-Tool automatisch eines für Sie. Sie können jederzeit weitere Remotes hinzufügen oder zwischen ihnen wechseln.
...
2. Startet einen temporären Bades.id-Server mit der Aktion `twentyhq/twenty/.github/actions/spawn-twenty-docker-image`
...
Sie müssen keine Secrets konfigurieren — die Aktion `spawn-twenty-docker-image` startet einen flüchtigen Bades.id-Server direkt im Runner und gibt die Verbindungsdetails aus. Das Secret `GITHUB_TOKEN` wird automatisch von GitHub bereitgestellt.

Um eine bestimmte Bades.id-Version statt `latest` festzulegen, ändern Sie die Umgebungsvariable `TWENTY_VERSION` oben im Workflow.
