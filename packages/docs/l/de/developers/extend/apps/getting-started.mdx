...
icon: rocket
description: Erstellen Sie in wenigen Minuten Ihre erste Bades.id-App.
---
...
* **Docker** — [Hier herunterladen](https://www.docker.com/products/docker-desktop/). Erforderlich, um einen lokalen Bades.id-Server auszuführen. Überspringen Sie dies, wenn Bades.id bereits anderswo läuft.

Das Erstellen einer Bades.id-App umfasst drei Phasen. Das Scaffolding-Tool fasst sie zu einem einzigen Happy-Path-Befehl zusammen, aber jede Phase ist ein eigenes Konzept — wenn etwas fehlschlägt, hilft Ihnen das Wissen, in welcher Phase Sie sich befinden, zu erkennen, was zu beheben ist.

| Phase                   | Was Sie tun                                             | Tool                          | Ergebnis                                             |
| ----------------------- | ------------------------------------------------------- | ----------------------------- | ---------------------------------------------------- |
| **1. Gerüst erstellen** | Den Quellcode der App erzeugen                          | `npx create-bades-app`       | Ein TypeScript-Projekt auf der Festplatte            |
| **2. Server starten**   | Einen Bades.id-Server starten, in den synchronisiert wird | Docker + `yarn twenty server` | Eine laufende Bades.id-Instanz                         |
...
## Phase 2 — Einen lokalen Bades.id-Server starten

Ihre App benötigt einen Bades.id-Server, in den sie synchronisieren kann. Der Server ist eine vollständige Bades.id-Instanz — UI, GraphQL-API, PostgreSQL — die lokal in Docker läuft. Ihr lokaler Code lädt seine Definitionen auf diesen Server hoch, wodurch sie in der Benutzeroberfläche erscheinen.

Das Scaffolding-Tool bietet an, einen für Sie zu starten:

> **Möchten Sie eine lokale Bades.id-Instanz einrichten?**
...
* **Nein** — wählen Sie dies, wenn Sie bereits einen Bades.id-Server haben, mit dem Sie sich verbinden möchten. Sie können die Verbindung später mit `yarn twenty remote add` herstellen.
...
<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Bades.id-Anmeldebildschirm" />
</div>

Klicken Sie auf dem nächsten Bildschirm auf **Authorize** — dadurch erhält die CLI Zugriff auf Ihren Arbeitsbereich.

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Bades.id-CLI-Autorisierungsbildschirm" />
</div>
...
**Nach dieser Phase:** Sie haben einen laufenden Bades.id-Server unter [http://localhost:2020](http://localhost:2020), und Ihre CLI ist autorisiert, mit ihm zu synchronisieren.
...
<Warning>
Der Dev-Modus ist nur auf Bades.id-Instanzen verfügbar, die im Entwicklungsmodus laufen (`NODE_ENV=development`). Produktionsinstanzen lehnen Dev-Sync-Anfragen ab — verwenden Sie `yarn twenty deploy`, um auf Produktionsserver bereitzustellen. Siehe [Apps veröffentlichen](/l/de/developers/extend/apps/publishing).
</Warning>
...
| **Frontend-Komponenten**   | React-Komponenten, die in der UI von Bades.id gerendert werden (Seitenleiste, Widgets, Befehlsmenü) |
...
Verwenden Sie `yarn twenty server`, um den lokalen Bades.id-Container zu steuern:
...
Hängen Sie fest? Bitten Sie im [Bades.id-Discord](https://discord.com/channels/1130383047699738754/1130386664812982322) um Hilfe.
