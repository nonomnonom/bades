---
title: Architektur
description: Wie Bades.id-Apps funktionieren — Sandboxing, Lebenszyklus und Bausteine.
icon: sitemap
---

Bades.id-Apps sind TypeScript-Pakete, die Ihren Arbeitsbereich mit benutzerdefinierten Objekten, Logik, UI-Komponenten und KI-Funktionen erweitern. Sie laufen auf der Bades.id-Plattform mit vollständigem Sandboxing und Berechtigungsverwaltung.
...
| **Frontend-Komponente**    | Sandboxed React-UI auf der Bades.id-Seite                           | [Frontend-Komponenten](/l/de/developers/extend/apps/front-components) |
...
* **Frontend-Komponenten** laufen in Web Workers mit Remote DOM — von der Hauptseite isoliert, rendern aber native DOM-Elemente (keine iframes). Sie kommunizieren über eine Message-Passing-Host-API mit Bades.id.
...
* **`yarn twenty dev`** — überwacht Ihre Quelldateien und synchronisiert Änderungen in Echtzeit mit einem verbundenen Bades.id-Server. Der typisierte API-Client wird automatisch neu erzeugt, wenn sich das Schema ändert.
...
<CardGroup cols={2}>
  ...
  <Card title="Frontend-Komponenten" icon="window-maximize" href="/l/de/developers/extend/apps/front-components">
    Sandboxed React-Komponenten innerhalb der UI von Bades.id.
  </Card>
