...
description: Vollständige Schritt-für-Schritt-Anleitung zum Formatieren deiner Daten für den Import in Bades.id.
---
...
**Dies ist der wichtigste Schritt.** Die Beispieldatei zeigt dir die exakten Spaltennamen und das Format, das Bades.id erwartet.
...
Bades.id erzwingt die Eindeutigkeit bestimmter Felder. Duplikate führen zu Importfehlern.
...
3. Prüfe auf Duplikate, die in Bades.id bereits existieren
...
* Muss eindeutig sein (keine Duplikate in der Datei oder in Bades.id)
...
* Muss innerhalb deiner Datei und in Bades.id eindeutig sein
...
* **Links / Link-Label**: `Bades.id`
* **Links / Link-URL**: `https://bades.id`

Für sekundäre Links verwenden Sie die Spalte **Links / Sekundäre Links**:

```
[{"url":"https://bades.id","label":"Bades.id"}]
```
...
* **Optional**: Bades.id erstellt IDs automatisch, wenn sie nicht angegeben werden
...
* Der übergeordnete Datensatz muss bereits in Bades.id vorhanden sein
...
## Schritt 6: Sicherstellen, dass Felder in Bades.id vorhanden sind
