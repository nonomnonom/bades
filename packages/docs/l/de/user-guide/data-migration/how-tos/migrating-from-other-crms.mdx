...
description: Schritt-für-Schritt-Anleitung zur Migration Ihrer Daten von jedem CRM zu Bades.id.
---

## Übersicht

Diese Anleitung führt Sie durch die Migration Ihrer Daten von jedem CRM zu Bades.id. Der Prozess umfasst die Überprüfung Ihrer Daten, die Vorbereitung Ihres Bades.id-Arbeitsbereichs, den Export aus Ihrem aktuellen System und den Import in Bades.id.
...
Erstellen Sie ein Zuordnungsdokument zwischen Ihrem aktuellen CRM und Bades.id:
...
## Schritt 3: Richten Sie Ihren Bades.id-Arbeitsbereich ein

Bereiten Sie Ihren Bades.id-Arbeitsbereich vor, bevor Sie Daten importieren:
...
Wenn Ihre Daten Benutzerreferenzen enthalten (Account Owner, Assignee usw.), müssen diese Benutzer vor dem Import in Bades.id existieren. Andernfalls können diese Beziehungen nicht zugeordnet werden.
</Warning>
...
Öffnen Sie jede exportierte CSV in einer Tabellenkalkulationsanwendung und bereiten Sie sie für Bades.id vor.
...
3. Überprüfen Sie, dass in Bades.id keine Duplikate vorhanden sind
...
Verwenden Sie `https://domain.com` (nicht `domain.com` oder `www.domain.com`). Dies entspricht dem Format von Bades.id und verhindert Duplikate, wenn Sie die E-Mail-/Kalendersynchronisierung verbinden.
</Warning>
...
2. Verwenden Sie dieselben E-Mail-Adressen, mit denen die Benutzer Ihrem Bades.id-Arbeitsbereich beigetreten sind
...
## Schritt 6: In Bades.id importieren
...
1. Navigieren Sie zum Objekt in Bades.id
...
* Bades.id beginnt, E-Mails mit Kontaktdatensätzen zu synchronisieren
...
<AccordionGroup>
  <Accordion title="Kann ich Workflows automatisch migrieren?">
    Derzeit nicht. Workflows müssen in Bades.id manuell neu erstellt werden.
  </Accordion>
...
Kontaktieren Sie [contact@bades.id](mailto:contact@bades.id) oder informieren Sie sich über unsere [Implementierungsservices](/l/de/user-guide/getting-started/capabilities/implementation-services).
