...
Nachdem Sie Ihre Datei hochgeladen und die Spalten zugeordnet haben, validiert Bades.id Ihre Daten:

1. **Validierung läuft** — Bades.id prüft jede Zeile auf Fehler
...
| ------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| Doppelte Werte, gelb hervorgehoben                                             | Wert existiert bereits in Bades.id oder kommt in Ihrer Datei doppelt vor |
...
**Option 3: Bades.id den vorhandenen Datensatz aktualisieren lassen**
...
3. Bades.id aktualisiert den vorhandenen Datensatz, statt ein Duplikat zu erstellen
...
Solange ein eindeutiger Bezeichner übereinstimmt, aktualisiert Bades.id den Datensatz.
</Note>
...
3. Prüfen Sie, ob Datensätze bereits in Bades.id existieren
...
Das bedeutet, dass es in Bades.id kein Unternehmen mit diesem spezifischen Bezeichner gibt.
...
Sie versuchen, auf einen Datensatz zu verweisen, der in Bades.id nicht existiert.
...
Eine Spalte in Ihrer Datei kann nicht zugeordnet werden, weil das Feld in Bades.id nicht existiert.
...
1. **Benutzer hat seine Einladung nicht angenommen** — der Benutzer existiert noch nicht in Bades.id
2. **Benutzer-ID aus altem System verwendet** — Bades.id kann IDs aus einem anderen System nicht zuordnen
3. **Falsches E-Mail-Format** — die E-Mail entspricht nicht dem Bades.id-Konto des Benutzers

### So beheben Sie den Fehler

1. Stellen Sie sicher, dass alle Benutzer **ihre Einladung** zu Ihrem Bades.id-Arbeitsbereich **angenommen haben**
2. Verwenden Sie die **E-Mail-Adresse** des Benutzers (nicht seinen Namen oder die alte System-ID)
3. Verwenden Sie dieselbe E-Mail, mit der sie Bades.id beigetreten sind
...
<Check>Domain-Format entspricht exakt dem Format von Bades.id</Check>
...
1. **Beispieldatei herunterladen** — sehen Sie das genaue Format, das Bades.id erwartet
