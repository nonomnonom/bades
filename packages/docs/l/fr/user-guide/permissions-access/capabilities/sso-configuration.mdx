...
L'authentification unique (SSO) permet aux membres de votre équipe de se connecter à Bades.id en utilisant le fournisseur d'identité de votre organisation. Cela offre :
...
Bades.id prend en charge le SSO avec :
...
* Accès administrateur à l'espace de travail Bades.id

<Note>
  **Pour les utilisateurs en auto-hébergement souhaitant configurer le SSO**, contactez contact@bades.id
</Note>
...
* Retirez-les de l'espace de travail Bades.id
...
* **Correspondance de groupes** : Faites correspondre les groupes IdP aux rôles Bades.id (si disponible)
* **Processus de départ** : Incluez Bades.id dans votre workflow de déprovisionnement
