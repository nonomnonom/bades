...
<AccordionGroup>
  <Accordion title="Quels rôles sont inclus par défaut avec Bades.id ?">
    Bades.id inclut par défaut les rôles **Admin** et **Member**. Vous pouvez créer des rôles personnalisés supplémentaires selon les besoins de votre équipe (par ex., Représentant commercial, Responsable, Utilisateur en lecture seule).
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Quelles autorisations d'actions sont disponibles ?">
    ...
    * **Envoyer des e-mails** : Possibilité d'envoyer des e-mails depuis Bades.id
...
<AccordionGroup>
  ...
  <Accordion title="Qu'est-ce que l'approvisionnement Just-in-Time (JIT) ?">
    Avec l'approvisionnement JIT, les comptes utilisateur sont automatiquement créés dans Bades.id lorsqu'une personne se connecte via SSO pour la première fois. Le rôle par défaut leur est attribué automatiquement.
  </Accordion>
