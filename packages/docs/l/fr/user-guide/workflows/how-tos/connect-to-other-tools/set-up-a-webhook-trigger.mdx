...
| **Intégrations personnalisées** | Votre application → automatisation dans Bades.id                 |
...
3. Vous recevrez une URL de webhook unique, par exemple :
   ```
   https://api.bades.id/webhooks/workflow/abc123...
   ```
...
```bash
curl -X POST https://api.bades.id/webhooks/workflow/abc123... \
