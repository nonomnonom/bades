---
title: Importer les données de produits dans Bades.id
...
Utilisez ce modèle pour garder Bades.id synchronisé avec les données produits de votre entrepôt de données (par ex. Snowflake, BigQuery, PostgreSQL).
...
5. **Upsert Record** : Créer ou mettre à jour dans Bades.id
...
export const main = async (params: {
  ...
  return {
    users: usersFormatted.map((user) => ({
      ...
      isBades.id: toBool(user.isBades.id),
    })),
  };
};
```
