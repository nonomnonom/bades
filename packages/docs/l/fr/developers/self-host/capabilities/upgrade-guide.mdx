...
1. Dans un terminal, sur l'hôte où Bades.id est en cours d'exécution, éteignez Bades.id: `docker compose down`
...
3. Remettez Bades.id en ligne avec `docker compose up -d`
...
Mettez à niveau votre instance Bades.id pour utiliser l'image v0.55
...
Ce problème est spécifique à cette version de Bades.id et ne devrait pas être nécessaire pour les futures mises à niveau.
...
Il doit toujours être dans la plage de la version `major.minor` de votre instance Bades.id actuelle, vous pouvez consulter la version de votre instance dans le panneau d'administration (à l'adresse `/settings/admin-panel`, accessible si votre utilisateur a la propriété `canAccessFullAdminPanel` définie comme vraie dans la base de données) ou en exécutant `echo $APP_VERSION` dans votre conteneur `twenty-server`.

Pour corriger une version d'espace de travail désynchronisée, vous devrez effectuer une mise à niveau à partir de la version correspondante de Bades.id en suivant le guide de mise à niveau correspondant séquentiellement, et ainsi de suite jusqu'à atteindre la version souhaitée.
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.52
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.51
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.50.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.44.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.43.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.42.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.41.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.40.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.35.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.34.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.33.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.32.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.31.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.30.0

**Changement significatif**:
Pour améliorer les performances, Bades.id nécessite désormais la configuration de cache redis. Nous avons mis à jour notre [docker-compose.yml](https://raw.githubusercontent.com/twentyhq/twenty/main/packages/twenty-docker/docker-compose.yml) pour refléter cela.
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.24.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.23.0
...
Mettez à jour votre instance Bades.id pour utiliser l'image v0.22.0
