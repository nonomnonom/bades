...
Ce guide fournit des instructions pas-à-pas pour installer et configurer l'application Bades.id à l'aide de Docker Compose. L'objectif est de simplifier le processus et d'éviter les erreurs courantes qui pourraient compromettre votre configuration.
...
Installez la dernière version stable de Bades.id avec une seule commande :
...
### Exposer Bades.id à un Accès Externe

Par défaut, Bades.id fonctionne sur `localhost` au port `3000`. Pour y accéder via un domaine externe ou une adresse IP, vous devez configurer le `SERVER_URL` dans votre fichier `.env`.
...
Nous vous recommandons vivement de mettre en place Bades.id derrière un proxy inverse avec terminaison SSL pour une sécurité et une fonctionnalité optimales.
