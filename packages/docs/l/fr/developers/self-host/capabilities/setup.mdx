...
<Warning>
  **Première installation ?** Suivez le [Guide d’installation de Docker Compose](/l/fr/developers/self-host/capabilities/docker-compose) pour faire fonctionner Bades.id, puis revenez ici pour la configuration.
</Warning>

Bades.id offre **deux modes de configuration** pour répondre à différents besoins de déploiement :
...
1. Accédez à votre instance Bades.id (généralement `http://localhost:3000`)
...
Par défaut, Bades.id fonctionne en **mode à espace de travail unique** — idéal pour la plupart des déploiements auto-hébergés où vous avez besoin d'une seule instance CRM pour votre organisation.
...
* Un espace de travail par instance Bades.id
...
Activez le mode multi-espaces de travail pour des déploiements de type SaaS où plusieurs équipes indépendantes ont besoin de leurs propres espaces de travail sur la même instance Bades.id.
...
<Warning>
  Les utilisateurs doivent avoir une [licence Microsoft 365](https://admin.microsoft.com/Adminportal/Home) pour pouvoir utiliser les API Calendrier et Messagerie. Ils ne pourront pas synchroniser leur compte sur Bades.id sans cela.
</Warning>
...
Bades.id prend en charge les fonctions logiques pour les flux de travail et la logique personnalisée. L'environnement d'exécution est configuré via la variable d'environnement `SERVERLESS_TYPE`.
...
| --------- | -------------------------- | ---------------------------------------------- | ------------------------------------ |
| Désactivé | `SERVERLESS_TYPE=DISABLED` | Désactiver complètement les fonctions logiques | N/A                                  |
...
<Note>
  Lorsque vous utilisez `SERVERLESS_TYPE=DISABLED`, toute tentative d'exécuter une fonction logique renverra une erreur. Cela est utile si vous souhaitez exécuter Bades.id sans la prise en charge des fonctions logiques.
</Note>
