...
Les webhooks envoient des données à vos systèmes en temps réel lorsque des événements se produisent dans Bades.id — aucune interrogation n'est nécessaire. Utilisez-les pour maintenir les systèmes externes synchronisés, déclencher des automatisations ou envoyer des alertes.
...
Bades.id envoie des webhooks pour ces types d'événements :
...
Bades.id signe chaque requête de webhook pour des raisons de sécurité. Validez les signatures pour garantir l'authenticité des requêtes.

### En-têtes

| En-tête                      | Description              |
| ---------------------------- | ------------------------ |
| `X-Bades.id-Webhook-Signature` | Signature HMAC SHA256    |
| `X-Bades.id-Webhook-Timestamp` | Horodatage de la requête |

### Étapes de validation

1. Récupérez l'horodatage depuis `X-Bades.id-Webhook-Timestamp`
2. Créez la chaîne : `{timestamp}:{JSON payload}`
3. Calculez le HMAC SHA256 en utilisant votre secret de webhook
4. Comparez avec `X-Bades.id-Webhook-Signature`
...
| **Déclencheur de webhook de workflow** | ENTRÉE    | Recevoir des données dans Bades.id depuis des systèmes externes                         |
