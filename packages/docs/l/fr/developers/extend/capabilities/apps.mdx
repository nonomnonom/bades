---
title: Applications Bades.id
description: Créez et gérez les personnalisations Bades.id sous forme de code.
---
...
Les applications vous permettent de créer et de gérer des personnalisations Bades.id **sous forme de code**. Au lieu de tout configurer via l’interface utilisateur, vous définissez votre modèle de données et vos fonctions logiques dans le code — ce qui accélère la création, la maintenance et le déploiement sur plusieurs espaces de travail.
...
* Un espace de travail Bades.id et une clé API (créez-en une sur https://app.bades.id/settings/api-webhooks)
...
Voir aussi : les pages de référence CLI pour [create-bades-app](https://www.npmjs.com/package/create-bades-app) et [twenty-sdk CLI](https://www.npmjs.com/package/twenty-sdk).

## Structure du projet (générée)

Lorsque vous exécutez `npx create-bades-app@latest my-twenty-app`, l’outil de scaffolding :
...
* `yarn app:dev` génère automatiquement le client Bades.id typé dans `node_modules/twenty-sdk/generated`.
...
<Note>
  **Les champs de base sont créés automatiquement.** Lorsque vous définissez un objet personnalisé, Bades.id ajoute automatiquement des champs standard tels que `name`, `createdAt`, `updatedAt`, `createdBy`, `position` et `deletedAt`. Vous n'avez pas besoin de les définir dans votre tableau `fields` — ajoutez uniquement vos champs personnalisés.
</Note>
...
`yarn app:dev` génère automatiquement le client Bades.id typé dans `node_modules/twenty-sdk/generated`. Utilisez-le dans vos fonctions :
...
Lorsque votre fonction s’exécute sur Bades.id, la plateforme injecte des identifiants sous forme de variables d’environnement avant l’exécution de votre code :

* `TWENTY_API_URL` : URL de base de l’API Bades.id ciblée par votre application.
...
Même si nous recommandons d’utiliser `create-bades-app` pour une expérience de démarrage optimale, vous pouvez également configurer un projet manuellement. N’installez pas la CLI globalement. Ajoutez plutôt `twenty-sdk` comme dépendance locale et reliez des scripts dans votre package.json :
...
* Impossible de se connecter au serveur : vérifiez l’URL de l’API et que le serveur Bades.id est accessible.
