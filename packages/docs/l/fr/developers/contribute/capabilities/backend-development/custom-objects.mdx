...
Les objets sont des structures qui vous permettent de stocker des données (enregistrements, attributs et valeurs) spécifiques à une organisation. Bades.id fournit à la fois des objets standard et personnalisés.

Les objets standard sont des objets intégrés avec un ensemble d'attributs disponibles pour tous les utilisateurs. Les exemples d'objets standard dans Bades.id incluent Société et Personne. Les objets standard ont des champs standard également disponibles pour tous les utilisateurs de Bades.id, comme Company.displayName.
