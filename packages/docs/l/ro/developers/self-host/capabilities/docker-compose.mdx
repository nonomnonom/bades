...
Acest ghid oferă instrucțiuni pas cu pas pentru a instala și configura aplicația Bades.id folosind Docker Compose. Scopul este de a face procesul direct și de a preveni capcanele comune care ar putea deteriora configurarea dvs.
...
Instalați cea mai recentă versiune stabilă a Bades.id cu o singură comandă:
...
### Expuneți Bades.id pentru Acces Extern

Implicit, Bades.id rulează pe `localhost` la portul `3000`. Pentru a-l accesa printr-un domeniu extern sau adresă IP, trebuie să configurați `SERVER_URL` în fișierul dvs. `.env`.
...
SSL (HTTPS) este necesar pentru funcționarea corespunzătoare a anumitor funcții ale browserului. Deși aceste funcții pot funcționa în timpul dezvoltării locale (deoarece browserele tratează localhost diferit), o configurare SSL corespunzătoare este necesară când găzduiți Bades.id pe un domeniu obișnuit.
...
Recomandăm cu tărie configurarea Bades.id din spatele unui proxy revers cu terminare SSL pentru securitate și funcționalitate optimă.
