...
icon: laptop-code
description: Ghidul pentru contribuitori (sau dezvoltatori curioși) care doresc să ruleze Bades.id local.
---
...
Înainte de a instala și utiliza Bades.id, asigurați-vă că instalați următoarele pe computerul dvs.:
...
Bades.id necesită un cache Redis pentru a oferi cea mai bună performanță.
...
## Pasul 8: Utilizați Bades.id

**Front-end**

Frontend-ul Bades.id va rula la [http://localhost:3001](http://localhost:3001).
Vă puteți loga folosind contul demo implicit: `tim@apple.dev` (parolă: `tim@apple.dev`)

**Back-end**

* Serverul Bades.id va fi operativ la [http://localhost:3000](http://localhost:3000)
