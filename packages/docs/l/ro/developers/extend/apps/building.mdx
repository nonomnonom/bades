---
title: Arhitectură
description: Cum funcționează aplicațiile Bades.id — izolarea (sandboxing), ciclul de viață și elementele de bază.
icon: sitemap
---

Aplicațiile Bades.id sunt pachete TypeScript care vă extind spațiul de lucru cu obiecte personalizate, logică, componente UI și capabilități AI. Acestea rulează pe platforma Bades.id, cu izolare completă (sandboxing) și controale de permisiuni.
...
| **Componentă front-end**         | UI React izolat în pagina Bades.id                            | [Componente front-end](/l/ro/developers/extend/apps/front-components) |
...
* **Componentele front-end** rulează în Web Workers folosind Remote DOM — izolate de pagina principală, dar randând elemente DOM native (nu iframes). Acestea comunică cu Bades.id printr-un API al gazdei bazat pe transmiterea de mesaje.
...
* **`yarn twenty dev`** — monitorizează fișierele sursă și sincronizează în timp real modificările către un server Bades.id conectat. Clientul API tipizat este regenerat automat atunci când schema se schimbă.
...
<CardGroup cols={2}>
  ...
  <Card title="Componente front-end" icon="window-maximize" href="/l/ro/developers/extend/apps/front-components">
    Componente React izolate în interfața Bades.id.
  </Card>
