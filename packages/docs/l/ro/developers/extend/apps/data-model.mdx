---
title: Model de date
description: Definiți obiecte, câmpuri, roluri și metadatele aplicației cu SDK-ul Bades.id.
icon: database
---
...
<Note>
**Câmpurile de bază sunt create automat.** Când definiți un obiect personalizat, Bades.id adaugă automat câmpuri standard
...
Utilizați `defineField()` pentru a adăuga câmpuri la obiecte pe care nu le dețineți — cum ar fi obiectele standard Bades.id (Person, Company etc.). sau obiecte din alte aplicații. Spre deosebire de câmpurile inline din `defineObject()`, câmpurile independente necesită un `objectUniversalIdentifier` pentru a specifica obiectul pe care îl extind:
...
Relațiile conectează obiectele între ele. În Bades.id, relațiile sunt întotdeauna bidirecționale — definiți ambele părți, iar fiecare parte o referențiază pe cealaltă.
...
Pentru a crea o relație cu un obiect Bades.id încorporat (Person, Company etc.), utilizați `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
