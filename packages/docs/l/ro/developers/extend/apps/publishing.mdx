...
title: Publicare
icon: încarcă
description: Distribuie aplicația ta Bades.id în marketplace sau implementeaz-o intern.
---
...
* **Implementați o arhivă tar** — încărcați aplicația direct pe un server Bades.id anume pentru uz intern sau privat.
* **Publică pe npm** — listează aplicația ta în marketplace-ul Bades.id pentru ca orice spațiu de lucru să o poată descoperi și instala.
...
Pentru aplicațiile pe care nu le dorești disponibile public — instrumente proprietare, integrări doar pentru enterprise sau build-uri experimentale — poți implementa un tarball direct pe un server Bades.id.
...
Dacă aplicația ta folosește o funcționalitate introdusă într-o anumită versiune de server Bades.id (de exemplu, furnizori OAuth adăugați în v2.3.0), ar trebui să declari versiunea minimă de server necesară aplicației folosind câmpul `engines.twenty` din `package.json`:
...
Aplicațiile generate cu `create-bades-app` vin, gata de utilizare, cu două fluxuri de lucru GitHub Actions, în `.github/workflows/`. Acestea sunt gata să ruleze imediat ce faci push al repozitoriului pe GitHub — nu este necesară nicio configurare suplimentară pentru CI, iar CD necesită doar un singur secret.
...
2. Pornește o instanță de test Bades.id izolată folosind acțiunea compozită `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` (echivalentul din CI al `yarn twenty server start --test`).
...
* `TWENTY_VERSION` (variabilă de mediu, implicit `latest`) — fixează versiunea serverului Bades.id folosită în CI editând acest parametru în `ci.yml`.
...
Implementează aplicația pe un server Bades.id configurat la fiecare push pe `main` și, opțional, dintr-un pull request când se aplică eticheta `deploy`.
...
| Setare                  | Unde                                                       | Scop                                                                                                               |
| ----------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `TWENTY_DEPLOY_URL`     | `env` în `cd.yml` (implicit `http://localhost:3000`)       | Serverul Bades.id la care se face implementarea. Modifică-l la URL-ul real al serverului înainte de prima utilizare. |
...
Publicarea pe npm face ca aplicația ta să poată fi descoperită în marketplace-ul Bades.id. Orice spațiu de lucru Bades.id poate răsfoi, instala și actualiza aplicațiile din marketplace direct din interfață.

### Cerințe

* Un cont [npm](https://www.npmjs.com)
* Cuvântul cheie `twenty-app` din array-ul `keywords` al fișierului `package.json` (adaugă-l manual — nu este inclus în mod implicit în șablonul `create-bades-app`)
...
Serverul Bades.id sincronizează catalogul marketplace-ului din registrul npm **la fiecare oră**.
...
<Note>
Dacă aplicația ta nu definește un `aboutDescription` în `defineApplication()`, piața va folosi automat fișierul `README.md` al pachetului tău de pe npm drept conținut pentru pagina Despre. Acest lucru înseamnă că poți menține un singur README atât pentru npm, cât și pentru piața Bades.id. Dacă vrei o descriere diferită în piață, setează explicit `aboutDescription`.
</Note>
...
Mergi la pagina **Setări > Aplicații** din Bades.id, unde pot fi parcurse și instalate atât aplicațiile din marketplace, cât și cele implementate prin tarball.
