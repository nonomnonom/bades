...
SDK-ul oferă API-uri programatice care vă permit să construiți, să distribuiți, să instalați și să dezinstalați aplicația din codul de test. Combinat cu [Vitest](https://vitest.dev/) și clienții API tipizați, puteți scrie teste de integrare care verifică faptul că aplicația funcționează cap-coadă împotriva unui server Bades.id real.
...
Asigurați-vă că serverul Bades.id local rulează, apoi:
...
<Note>
Acest lucru este diferit de `yarn twenty server logs`, care afișează jurnalele containerului Docker. `yarn twenty logs` afișează jurnalele de execuție ale funcțiilor aplicației de pe serverul Bades.id.
</Note>
...
Un „remote” este un server Bades.id la care se conectează aplicația. În timpul configurării, Scaffolderul creează automat unul pentru dvs. Puteți adăuga mai multe remote-uri sau comuta între ele oricând.
...
1. Preia codul
2. Pornește un server Bades.id temporar folosind acțiunea `twentyhq/twenty/.github/actions/spawn-twenty-docker-image`
...
Nu trebuie să configurați niciun secret — acțiunea `spawn-twenty-docker-image` pornește un server Bades.id efemer direct în runner și oferă detaliile de conectare. Secretul `GITHUB_TOKEN` este furnizat automat de GitHub.

Pentru a fixa o versiune Bades.id specifică în loc de `latest`, modificați variabila de mediu `TWENTY_VERSION` din partea de sus a workflow-ului.
