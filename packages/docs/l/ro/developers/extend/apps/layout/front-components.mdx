...
description: Construiți componente React care se afișează în interfața Bades.id, cu izolare în sandbox.
icon: window-maximize
---

Componentele front-end sunt componente React care se afișează direct în interfața Bades.id. Rulează într-un **Web Worker** izolat folosind Remote DOM — codul este izolat (sandboxed), dar se redă nativ în pagină, nu într-un iframe.

## Unde pot fi utilizate componentele front-end

Componentele front-end pot fi afișate în două locații în cadrul Bades.id:
...
Deoarece componenta returnează `null`, Bades.id omite redarea unui container pentru ea — nu apare spațiu gol în layout. Componenta are în continuare acces la toate hook-urile și la API-ul de comunicare cu gazda.
...
| Variabilă                 | Descriere                                                |
| ------------------------- | -------------------------------------------------------- |
| `TWENTY_API_URL`          | URL de bază al API-ului Bades.id                           |
...
* **Componente Bades.id UI** — import din `twenty-sdk/ui` (Button, Tag, Status, Chip, Avatar și altele)
