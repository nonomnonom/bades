...
After uploading your file and mapping columns, Bades.id validates your data:

1. **Validation runs** — Bades.id checks each row for errors
...
| --------------------------------------------------------------------- | ------------------------------------------------------------ |
| Duplicate values highlighted in yellow                                | Value already exists in Bades.id or appears twice in your file |
...
**Option 3: Let Bades.id update the existing record**

1. Ensure your file includes a unique identifier (`email`, `domain`, or `id`)
2. Map the unique identifier field
3. Bades.id will update the existing record instead of creating a duplicate
...
As long as one unique identifier matches, Bades.id updates the record.
</Note>
...
3. Check if records already exist in Bades.id
...
This means there is no Company in Bades.id with that specific identifier.
...
You're trying to link to a record that doesn't exist in Bades.id.
...
A column in your file can't be mapped because the field doesn't exist in Bades.id.
...
1. **User hasn't accepted their invitation** — the user doesn't exist in Bades.id yet
2. **Using user ID from old system** — Bades.id can't match IDs from another system
3. **Wrong email format** — the email doesn't match the user's Bades.id account

### How to Fix

1. Ensure all users have **accepted their invitation** to your Bades.id workspace
2. Use the user's **email address** (not their name or old system ID)
3. Use the same email they used to join Bades.id
...
<Check>Domain format matches Bades.id's format exactly</Check>
...
1. **Download the sample file** — see the exact format Bades.id expects
