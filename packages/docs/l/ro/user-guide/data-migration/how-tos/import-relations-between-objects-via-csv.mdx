...
Înregistrarea părinte (partea „unu”) trebuie să existe în Bades.id înainte de a o putea referi.
...
1. Exportați înregistrările părinte din Bades.id
...
Folosiți `https://domain.com` (nu doar `domain.com`). Acest lucru corespunde modului în care Bades.id stochează domeniile companiilor și previne erorile de potrivire.
</Warning>
...
Bades.id va lega automat fiecare înregistrare copil de părintele corespunzător.
...
| **Înregistrări inexistente**           | Înregistrarea părinte nu există în Bades.id            | Importați mai întâi înregistrările părinte sau verificați greșelile de tastare |
| **Nepotrivire de majuscule/minuscule** | `Acme.com` în fișier, dar `acme.com` în Bades.id       | Asigurați potrivirea exactă a majusculelor/minusculelor                        |
...
<AccordionGroup>
  <Accordion title="Ce se întâmplă dacă înregistrarea mea părinte nu are un domeniu sau un email?">
    Aveți două opțiuni:
    1. Folosiți `id`-ul Bades.id (exportați înregistrările părinte pentru a obține ID-urile lor)
