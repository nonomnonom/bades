...
description: Ghid pas cu pas pentru migrarea datelor din orice CRM către Bades.id.
---

## Prezentare generală

Acest ghid vă ghidează prin procesul de migrare a datelor din orice CRM către Bades.id. Procesul implică auditarea datelor, pregătirea spațiului de lucru Bades.id, exportul din sistemul curent și importul în Bades.id.
...
Creați un document de mapare între CRM-ul curent și Bades.id:
...
## Pasul 3: Configurați spațiul dvs. de lucru Bades.id

Înainte de a importa date, pregătiți spațiul dvs. de lucru Bades.id:
...
Dacă datele dvs. includ referințe la utilizatori (Proprietar cont, Atribuit etc.), acei utilizatori trebuie să existe în Bades.id înainte de import. În caz contrar, acele relații nu pot fi mapate.
</Warning>
...
Deschideți fiecare CSV exportat într-o aplicație de calcul tabelar și pregătiți-l pentru Bades.id.
...
3. Verificați că nu există deja duplicate în Bades.id
...
Folosiți `https://domain.com` (nu `domain.com` sau `www.domain.com`). Acest lucru corespunde formatului Bades.id și previne duplicatele când conectați sincronizarea e-mailului/calendarului.
</Warning>
...
2. Folosiți aceleași adrese de e-mail pe care utilizatorii le-au folosit pentru a se alătura spațiului dvs. de lucru Bades.id
...
1. Navigați la obiect în Bades.id
...
* Bades.id va începe să sincronizeze e-mailurile cu înregistrările de contact
...
<AccordionGroup>
  <Accordion title="Pot migra fluxurile de lucru automat?">
    Momentan, nu. Fluxurile de lucru trebuie recreate manual în Bades.id.
  </Accordion>
...
Contactați [contact@bades.id](mailto:contact@bades.id) sau explorați [Serviciile noastre de implementare](/l/ro/user-guide/getting-started/capabilities/implementation-services).
