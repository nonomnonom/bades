...
title: Pubblicazione
icon: carica
description: Distribuisci la tua app Bades.id nel marketplace oppure distribuiscila internamente.
---
...
* **Distribuisci un tarball** — carica la tua app direttamente su un server Bades.id specifico per uso interno o privato.
* **Pubblica su npm** — elenca la tua app nel marketplace di Bades.id affinché qualsiasi spazio di lavoro possa scoprirla e installarla.
...
Per le app che non vuoi rendere pubbliche — strumenti proprietari, integrazioni solo aziendali o build sperimentali — puoi distribuire un tarball direttamente su un server Bades.id.
...
2. Avvia un'istanza di test isolata di Bades.id utilizzando l'azione composita `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` (l'equivalente per la CI di `yarn twenty server start --test`).
...
* `TWENTY_VERSION` (variabile di ambiente, predefinito `latest`) — fissa la versione del server Bades.id usata nella CI modificando questo valore in `ci.yml`.
...
Esegue il deploy della tua app su un server Bades.id configurato a ogni push su `main` e, facoltativamente, da una pull request quando viene applicata l'etichetta `deploy`.
...
| ----------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `env` in `cd.yml` (predefinito `http://localhost:3000`)          | Il server Bades.id su cui effettuare il deploy. Modificalo con l'URL reale del tuo server prima del primo utilizzo. |
...
La pubblicazione su npm rende la tua app scopribile nel marketplace di Bades.id. Qualsiasi spazio di lavoro Bades.id può sfogliare, installare e aggiornare le app del marketplace direttamente dall'interfaccia utente.

### Requisiti

* Un account [npm](https://www.npmjs.com)
* La parola chiave `twenty-app` nell'array `keywords` del tuo `package.json` (aggiungila manualmente — non è inclusa per impostazione predefinita nel template `create-bades-app`)
...
Il server Bades.id sincronizza il proprio catalogo del marketplace dal registro npm **ogni ora**.
...
<Note>
Se la tua app non definisce un `aboutDescription` in `defineApplication()`, il marketplace userà automaticamente il `README.md` del tuo pacchetto su npm come contenuto della pagina Informazioni. Questo significa che puoi mantenere un unico README sia per npm sia per il marketplace di Bades.id. Se desideri una descrizione diversa nel marketplace, imposta esplicitamente `aboutDescription`.
</Note>
...
Vai alla pagina **Impostazioni > Applicazioni** in Bades.id, dove è possibile sfogliare e installare sia le app del marketplace sia quelle distribuite tramite tarball.
