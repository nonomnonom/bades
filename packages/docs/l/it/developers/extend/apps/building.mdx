---
title: Architettura
description: Come funzionano le app di Bades.id — sandboxing, ciclo di vita e componenti di base.
icon: sitemap
---

Le app di Bades.id sono pacchetti TypeScript che estendono il tuo spazio di lavoro con oggetti personalizzati, logica, componenti dell'interfaccia utente (UI) e funzionalità di IA. Vengono eseguite sulla piattaforma Bades.id con sandboxing completo e controlli delle autorizzazioni.
...
| **Componente front-end**         | UI React in sandbox nella pagina di Bades.id             | [Componenti front-end](/l/it/developers/extend/apps/front-components) |
...
* **I componenti front-end** vengono eseguiti in Web Workers utilizzando il Remote DOM — isolati dalla pagina principale ma renderizzando elementi DOM nativi (non iframe). Comunicano con Bades.id tramite un'API host basata sul passaggio di messaggi.
...
* **`yarn twenty dev`** — monitora i file sorgente e sincronizza in tempo reale le modifiche su un server Bades.id connesso. Il client API tipizzato viene rigenerato automaticamente quando lo schema cambia.
...
<CardGroup cols={2}>
  ...
  <Card title="Componenti front-end" icon="window-maximize" href="/l/it/developers/extend/apps/front-components">
    Componenti React in sandbox nell'UI di Bades.id.
  </Card>
