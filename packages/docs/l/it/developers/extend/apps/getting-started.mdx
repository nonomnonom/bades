...
* **Docker** — [Scarica qui](https://www.docker.com/products/docker-desktop/). Necessario per eseguire un'istanza locale di Bades.id. Non necessario se hai già un server Bades.id in esecuzione.
...
### Configura un'istanza locale di Bades.id

Lo strumento di scaffolding chiederà:

> **Vuoi configurare un'istanza locale di Bades.id?**

* **Digita `yes`** (consigliato) — Questo scarica l'immagine Docker `twenty-app-dev` e avvia un server Bades.id locale sulla porta `2020`. Assicurati che Docker sia in esecuzione prima di continuare.
* **Digita `no`** — Sceglilo se hai già un server Bades.id in esecuzione in locale.
...
Successivamente si aprirà una finestra del browser con la pagina di accesso di Bades.id. Accedi con l'account demo preconfigurato:

* **Email:** `tim@apple.dev`
* **Password:** `tim@apple.dev`

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Schermata di accesso di Bades.id" />
</div>
...
<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Schermata di autorizzazione della CLI di Bades.id" />
</div>
...
Questo controlla i file sorgente, ricompila a ogni modifica e sincronizza automaticamente la tua app con il server Bades.id locale. Dovresti vedere un pannello di stato in tempo reale nel terminale.
...
<Warning>
La modalità di sviluppo è disponibile solo sulle istanze di Bades.id in esecuzione in modalità sviluppo (`NODE_ENV=development`). Le istanze di produzione rifiutano le richieste di sincronizzazione in modalità sviluppo. Usa `yarn twenty deploy` per distribuire sui server di produzione — vedi [Pubblicazione delle app](/l/it/developers/extend/apps/publishing) per i dettagli.
</Warning>
...
Entrambe le modalità richiedono un server Bades.id in esecuzione in modalità di sviluppo e un remote autenticato — si applicano gli stessi prerequisiti.

### Visualizza la tua app in Bades.id
...
Fai clic su **My twenty app** per aprire la sua **registrazione dell'applicazione**. Una registrazione è un record a livello di server che descrive la tua app — il suo nome, identificatore univoco, credenziali OAuth e origine (locale, npm o tarball). Risiede sul server, non all'interno di uno spazio di lavoro specifico. Quando installi un'app in uno spazio di lavoro, Bades.id crea un'**applicazione** con ambito dello spazio di lavoro che rimanda a questa registrazione. Una registrazione può essere installata in più spazi di lavoro sullo stesso server.
...
| **Componenti front-end** | Componenti React che vengono renderizzati all'interno dell'UI di Bades.id (pannello laterale, widget, menu comandi) |
...
```bash filename="Terminal"
npx create-bades-app@latest my-twenty-app --example postcard
```
...
Lo scaffolder ha già avviato per te un server Bades.id locale. Per gestirlo in seguito, usa `yarn twenty server`:
...
Se preferisci configurare tutto manualmente invece di usare `create-bades-app`, puoi farlo in due passaggi.
...
Ancora bloccato? Chiedi aiuto su [Discord di Bades.id](https://discord.com/channels/1130383047699738754/1130386664812982322).
