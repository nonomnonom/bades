---
title: Modello dati
description: Definisci oggetti, campi, ruoli e metadati dell'applicazione con Bades.id SDK.
icon: database
---
...
<Note>
**I campi base vengono creati automaticamente.** Quando definisci un oggetto personalizzato, Bades.id aggiunge automaticamente i campi standard
...
Usa `defineField()` per aggiungere campi a oggetti che non possiedi — come gli oggetti standard di Bades.id (Person, Company, ecc.) o oggetti di altre app. A differenza dei campi inline in `defineObject()`, i campi autonomi richiedono un `objectUniversalIdentifier` per specificare quale oggetto estendono:
...
Le relazioni collegano gli oggetti tra loro. In Bades.id, le relazioni sono sempre **bidirezionali** — definisci entrambi i lati e ciascun lato fa riferimento all'altro.
...
Per creare una relazione con un oggetto Bades.id integrato (Person, Company, ecc.), usa `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
