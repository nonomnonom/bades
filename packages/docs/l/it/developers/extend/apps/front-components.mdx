...
description: Crea componenti React che vengono renderizzati all'interno della UI di Bades.id con isolamento in sandbox.
icon: window-maximize
---

I componenti front-end sono componenti React che vengono renderizzati direttamente all'interno della UI di Bades.id. Vengono eseguiti in un **Web Worker** isolato utilizzando Remote DOM — il tuo codice è in sandbox ma viene renderizzato in modo nativo nella pagina, non in un iframe.

## Dove possono essere utilizzati i componenti front.

I componenti front possono essere renderizzati in due posizioni all'interno di Bades.id:
...
Poiché il componente restituisce `null`, Bades.id evita di renderizzare un contenitore per esso — non appare alcuno spazio vuoto nel layout. Il componente ha comunque accesso a tutti gli hook e all'API di comunicazione con l'host.
...
* **Componenti Bades.id UI** — importali da `twenty-sdk/ui` (Button, Tag, Status, Chip, Avatar e altro)
