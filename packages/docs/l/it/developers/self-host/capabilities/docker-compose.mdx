...
Questa guida fornisce istruzioni passo passo per installare e configurare l'applicazione Bades.id usando Docker Compose. L'obiettivo è rendere il processo semplice ed evitare gli errori comuni che potrebbero compromettere il tuo setup.
...
Installa l'ultima versione stabile di Bades.id con un unico comando:
...
### Esponi Bades.id per Accesso Esterno

Per impostazione predefinita, Bades.id gira su `localhost` alla porta `3000`. Per accedervi tramite un dominio esterno o indirizzo IP devi configurare il `SERVER_URL` nel tuo file `.env`.
...
Raccomandiamo fortemente di configurare Bades.id dietro un proxy inverso con terminazione SSL per una sicurezza e funzionalità ottimali.
