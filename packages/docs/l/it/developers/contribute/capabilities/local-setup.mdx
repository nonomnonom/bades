...
icon: laptop-code
description: La guida per i collaboratori (o sviluppatori curiosi) che vogliono eseguire Bades.id localmente.
---
...
Prima di poter installare e usare Bades.id, assicurati di installare quanto segue sul tuo computer:
...
Bades.id richiede una cache Redis per offrire le migliori prestazioni.
...
## Passo 8: Utilizza Bades.id

**Frontend**

Il frontend di Bades.id sarà in esecuzione su [http://localhost:3001](http://localhost:3001).
Puoi accedere utilizzando l'account demo predefinito: `tim@apple.dev` (password: `tim@apple.dev`)

**Backend**

* Il server di Bades.id sarà attivo e funzionante su [http://localhost:3000](http://localhost:3000)
