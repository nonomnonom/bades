...
description: Quando e come usare le API di Bades.id per importazioni di dati su larga scala.
---

## Panoramica

Bades.id offre sia **GraphQL** che **API REST** per l'importazione programmatica dei dati. Usa l'API quando l'importazione CSV non è pratica per il tuo volume di dati o quando hai bisogno di importazioni automatizzate e ricorrenti.
...
Bades.id applica limiti di frequenza per garantire la stabilità del sistema:
...
Bades.id supporta due tipi di API:
...
* Verifica che i formati soddisfino i requisiti di Bades.id (vedi [Mappatura dei campi](/l/it/user-guide/data-migration/capabilities/field-mapping))
...
* Verifica che i dati compaiano correttamente in Bades.id
...
Contattaci a [contact@bades.id](mailto:contact@bades.id) oppure scopri i nostri [Servizi di implementazione](/l/it/user-guide/getting-started/capabilities/implementation-services).
