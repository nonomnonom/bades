...
Dopo aver caricato il file e mappato le colonne, Bades.id convalida i tuoi dati:

1. **Esecuzione della convalida** — Bades.id controlla ogni riga per individuare errori
...
| ---------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| Valori duplicati evidenziati in giallo                                             | Il valore esiste già in Bades.id o compare due volte nel tuo file |
...
**Opzione 3: Consenti a Bades.id di aggiornare il record esistente**

1. Assicurati che il tuo file includa un identificatore univoco (`email`, `domain` o `id`)
2. Mappa il campo identificatore univoco
3. Bades.id aggiornerà il record esistente invece di creare un duplicato
...
Finché almeno un identificatore univoco corrisponde, Bades.id aggiorna il record.
</Note>
...
3. Verifica se i record esistono già in Bades.id
...
Ciò significa che non esiste alcuna Azienda in Bades.id con quello specifico identificatore.
...
Stai tentando di collegarti a un record che non esiste in Bades.id.
...
Una colonna del tuo file non può essere mappata perché il campo non esiste in Bades.id.
...
1. **L'utente non ha accettato l'invito** — l'utente non esiste ancora in Bades.id
2. **Uso dell'ID utente dal vecchio sistema** — Bades.id non può abbinare gli ID di un altro sistema
3. **Formato email errato** — l'email non corrisponde all'account Bades.id dell'utente

### Come risolvere

1. Assicurati che tutti gli utenti abbiano **accettato il loro invito** al tuo spazio di lavoro Bades.id
2. Usa l'**indirizzo email** dell'utente (non il nome o l'ID del vecchio sistema)
3. Usa la stessa email che hanno utilizzato per aderire a Bades.id
...
<Check>Il formato del dominio corrisponde esattamente al formato di Bades.id</Check>
...
1. **Scarica il file di esempio** — vedi il formato esatto che Bades.id si aspetta
