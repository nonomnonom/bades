...
description: Guida completa passo passo per formattare i tuoi dati per l'importazione in Bades.id.
---
...
**Questo è il passaggio più importante.** Il file di esempio ti mostra i nomi delle colonne e il formato esatti che Bades.id si aspetta.
...
Bades.id impone l'unicità su alcuni campi. I duplicati causeranno errori di importazione.
...
3. Verifica la presenza di duplicati già esistenti in Bades.id
...
* Deve essere univoco (nessun duplicato nel file o in Bades.id)
...
* Deve essere univoco all'interno del tuo file e in Bades.id
...
* **Link / Etichetta del link**: `Bades.id`
* **Link / URL del link**: `https://bades.id`
...
* **Opzionale**: Bades.id genera automaticamente gli ID se non forniti
...
* Il record padre deve già esistere in Bades.id
...
## Passaggio 6: Verifica che i campi esistano in Bades.id
