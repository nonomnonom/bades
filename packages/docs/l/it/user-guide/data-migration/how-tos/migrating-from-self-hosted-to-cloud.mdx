...
description: Guida passo-passo per migrare la tua istanza Bades.id self-hosted verso Bades.id Cloud.
---

## Panoramica

Questa guida ti accompagna nella migrazione dei tuoi dati da un'istanza Bades.id self-hosted a Bades.id Cloud. Il processo prevede la configurazione dello spazio di lavoro cloud, l'esportazione dei dati e la loro reimportazione.
...
1. Vai su [app.bades.id](https://app.bades.id) e crea un nuovo spazio di lavoro
...
Per migrazioni complesse o set di dati di grandi dimensioni, contattaci all'indirizzo [contact@bades.id](mailto:contact@bades.id) oppure scopri i nostri [Servizi di Implementazione](/l/it/user-guide/getting-started/capabilities/implementation-services).
