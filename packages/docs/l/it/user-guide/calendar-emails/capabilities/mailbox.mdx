...
description: Comprendere le funzionalità di integrazione delle email in Bades.id.
---
...
Bades.id collega automaticamente le email dalle tue caselle di posta connesse ai relativi record CRM, mantenendo la cronologia di comunicazione in un unico posto.
...
Bades.id importa email a circa **400 messaggi al minuto**. Questa velocità è attualmente limitata dalle limitazioni di frequenza dell'API di Gmail (e vale in modo analogo per altri provider).
...
* **Email mancanti**: Verifica se:
  ...
  * La creazione automatica dei contatti è disabilitata (le email necessitano di record Bades.id esistenti)
