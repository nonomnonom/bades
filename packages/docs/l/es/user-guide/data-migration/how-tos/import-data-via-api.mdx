...
description: Cuándo y cómo usar las API de Bades.id para importaciones de datos a gran escala.
---

## Resumen

Bades.id ofrece tanto **GraphQL** como **API REST** para la importación programática de datos. Utiliza la API cuando la importación mediante CSV no sea práctica para tu volumen de datos o cuando necesites importaciones automatizadas y recurrentes.
...
Bades.id aplica límites de tasa para garantizar la estabilidad del sistema:
...
Bades.id admite dos tipos de API:
...
* Verifica que los formatos cumplan los requisitos de Bades.id (consulta [Asignación de campos](/l/es/user-guide/data-migration/capabilities/field-mapping))
...
* Verifica que los datos aparezcan correctamente en Bades.id
...
Contáctanos en [contact@bades.id](mailto:contact@bades.id) o explora nuestros [Servicios de implementación](/l/es/user-guide/getting-started/capabilities/implementation-services).
