...
### Relaciones comunes en Bades.id
...
El padre (lado "uno") debe existir en Bades.id antes de poder hacer referencia a él.
...
1. Exporte los registros padre desde Bades.id
...
<Warning>
  **¡El formato del dominio importa!**
  ...
  Use `https://domain.com` (no solo `domain.com`). Esto coincide con cómo Bades.id almacena los dominios de las Empresas y evita errores de coincidencia.
</Warning>
...
Bades.id vinculará automáticamente cada registro hijo con el padre correspondiente.
...
| **Registros inexistentes**                | El registro padre no existe en Bades.id                | Importe primero los registros padre o verifique si hay errores tipográficos |
| **Diferencia de mayúsculas y minúsculas** | `Acme.com` en el archivo pero `acme.com` en Bades.id   | Asegure una coincidencia exacta de mayúsculas y minúsculas                  |
...
<AccordionGroup>
  <Accordion title="¿Qué pasa si mi registro padre no tiene dominio ni correo electrónico?">
    ...
    1. Use el `id` de Bades.id (exporte los registros padre para obtener sus IDs)
