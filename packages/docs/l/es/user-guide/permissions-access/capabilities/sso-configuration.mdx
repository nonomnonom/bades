...
El inicio de sesión único (SSO) permite que los miembros de su equipo inicien sesión en Bades.id utilizando el proveedor de identidad de su organización. Esto proporciona:
...
* Acceso de administrador al espacio de trabajo de Bades.id

<Note>
  **Para usuarios de autoalojamiento que deseen configurar SSO**, comuníquese con contact@bades.id
</Note>
...
* **Entity ID**: Proporcionado por Bades.id
* **ACS URL**: La URL de retorno para la autenticación
* **Certificado**: Para comunicación segura

#### 4. Introduzca los detalles del proveedor en Bades.id
...
* Elimínelos del espacio de trabajo de Bades.id
...
* **Asignación de grupos**: Asigne los grupos del IdP a roles de Bades.id (si está disponible)
* **Proceso de desvinculación**: Incluya Bades.id en su flujo de trabajo de desaprovisionamiento
