...
Esta guía proporciona instrucciones paso a paso para instalar y configurar la aplicación Bades.id utilizando Docker Compose. El objetivo es simplificar el proceso y prevenir errores comunes que podrían arruinar tu configuración.
...
Instala la última versión estable de Bades.id con un solo comando:
...
### Exponer Bades.id para acceso externo

Por defecto, Bades.id se ejecuta en `localhost` en el puerto `3000`. Para acceder a él mediante un dominio externo o dirección IP, necesitas configurar `SERVER_URL` en tu archivo `.env`.
...
Recomendamos encarecidamente configurar Bades.id detrás de un proxy inverso con terminación SSL para una seguridad y funcionalidad óptimas.
