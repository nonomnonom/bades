...
description: La guía para los colaboradores (o desarrolladores curiosos) que quieren ejecutar Bades.id localmente.
---
...
<Tabs>
  <Tab title="Linux y MacOS">
    Antes de que puedas instalar y usar Bades.id, asegúrate de instalar lo siguiente en tu computadora:
...
Bades.id requiere un caché de redis para proporcionar el mejor rendimiento
...
El frontend de Bades.id estará ejecutándose en [http://localhost:3001](http://localhost:3001).
Puede iniciar sesión usando la cuenta demo por defecto: `tim@apple.dev` (contraseña: `tim@apple.dev`)

**Backend**

* El servidor de Bades.id estará operativo en [http://localhost:3000](http://localhost:3000)
