---
title: Aplicaciones de Bades.id
description: Crea y gestiona personalizaciones de Bades.id como código.
---
...
Las aplicaciones te permiten crear y administrar personalizaciones de Bades.id **como código**. En lugar de configurar todo a través de la interfaz de usuario, defines tu modelo de datos y funciones de lógica en código, lo que hace más rápido crear, mantener y desplegar en múltiples espacios de trabajo.
...
* Node.js 24+ y Yarn 4
* Un espacio de trabajo de Bades.id y una clave de API (créala en https://app.bades.id/settings/api-webhooks)
...
Consulta también: las páginas de referencia de la CLI para [create-bades-app](https://www.npmjs.com/package/create-bades-app) y [twenty-sdk CLI](https://www.npmjs.com/package/twenty-sdk).

## Estructura del proyecto (generada)

Cuando ejecutas `npx create-bades-app@latest my-twenty-app`, el generador:
...
* `yarn app:dev` genera automáticamente el cliente Bades.id tipado en `node_modules/twenty-sdk/generated`.
...
<Note>
  **Los campos base se crean automáticamente.** Cuando defines un objeto personalizado, Bades.id añade automáticamente campos estándar como `name`, `createdAt`, `updatedAt`, `createdBy`, `position` y `deletedAt`. No necesitas definir estos en tu matriz `fields` — solo agrega tus campos personalizados.
</Note>
...
export default defineApplication({
  ...
  displayName: 'My Bades.id App',
  description: 'My first Bades.id app',
  icon: 'IconWorld',
...
import Bades.id, { type Person } from '~/generated';

const handler = async (params: RoutePayload) => {
  const client = new Bades.id(); // generated typed client
...
`yarn app:dev` genera automáticamente el cliente Bades.id tipado en `node_modules/twenty-sdk/generated`. Úsalo en tus funciones:

```typescript
import Bades.id from '~/generated';

const client = new Bades.id();
...
Cuando tu función se ejecuta en Bades.id, la plataforma inyecta credenciales como variables de entorno antes de que tu código se ejecute:

* `TWENTY_API_URL`: URL base de la API de Bades.id a la que apunta tu aplicación.
...
Aunque recomendamos usar `create-bades-app` para la mejor experiencia de inicio, también puedes configurar un proyecto manualmente. No instales la CLI globalmente. En su lugar, agrega `twenty-sdk` como dependencia local y conecta scripts en tu package.json:
...
* No se puede conectar al servidor: verifica la URL de la API y que el servidor de Bades.id sea accesible.
