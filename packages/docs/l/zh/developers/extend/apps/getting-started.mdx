...
title: 开始使用
icon: rocket
description: 几分钟内创建你的第一个 Bades.id 应用。
---
...
* **Docker** — [在此下载](https://www.docker.com/products/docker-desktop/)。 运行本地 Bades.id 服务器所需。 如果你已经在其他地方运行了 Bades.id，请跳过。

构建一个 Bades.id 应用包含三个阶段。 脚手架工具将它们合并为一个理想路径的命令，但每个阶段都是独立的概念——当出现问题时，知道自己处于哪个阶段可以指明需要修复什么。

| 阶段           | 你要做什么                | 工具                            | 结果                   |
| ------------ | -------------------- | ----------------------------- | -------------------- |
| **1. 脚手架**   | 生成应用的源代码             | `npx create-bades-app`       | 磁盘上的一个 TypeScript 项目 |
| **2. 运行服务器** | 启动一个 Bades.id 服务器以进行同步 | Docker + `yarn twenty server` | 一个正在运行的 Bades.id 实例    |
...
## 阶段 2 — 运行本地 Bades.id 服务器

你的应用需要一个 Bades.id 服务器来进行同步。 该服务器是一个完整的 Bades.id 实例——包含 UI、GraphQL API、PostgreSQL——在本地的 Docker 中运行。 你的本地代码会将其定义上传到该服务器，从而使其显示在 UI 中。
...
* **否** — 如果你已经有一个想要连接的 Bades.id 服务器，请选择此项。 你可以稍后通过 `yarn twenty remote add` 将其连接起来。
...
<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Bades.id 登录界面" />
</div>

在下一屏点击 **Authorize** —— 这将授予 CLI 访问你工作区的权限。

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Bades.id CLI 授权界面" />
</div>
...
**完成此阶段后：** 你在 [http://localhost:2020](http://localhost:2020) 上拥有一个正在运行的 Bades.id 服务器，且你的 CLI 已获授权可与其同步。
...
<Warning>
开发模式仅适用于以开发模式运行的 Bades.id 实例（`NODE_ENV=development`）。 生产实例会拒绝开发同步请求——请使用 `yarn twenty deploy` 部署到生产服务器。 参见[发布应用](/l/zh/developers/extend/apps/publishing)。
</Warning>
...
| **前端组件**   | 在 Bades.id 的 UI 内渲染的 React 组件（侧边面板、小部件、命令菜单） |
...
使用 `yarn twenty server` 控制本地的 Bades.id 容器：
...
卡住了吗？ 在 [Bades.id 的 Discord](https://discord.com/channels/1130383047699738754/1130386664812982322) 上提问。
