---
title: 前端组件
description: 构建可在 Bades.id 的 UI 中渲染并具备沙盒隔离的 React 组件。
icon: window-maximize
---

前端组件是直接在 Bades.id 的 UI 内渲染的 React 组件。 它们在使用 Remote DOM 的**隔离 Web Worker**中运行——你的代码在沙盒中执行，但会原生渲染到页面中，而非在 iframe 里。

## 前端组件可用位置

在 Bades.id 中，前端组件可在两个位置进行渲染：
...
由于该组件返回 `null`，Bades.id 会跳过为其渲染容器——布局中不会出现空白区域。 该组件仍可访问所有 hooks 和宿主通信 API。
...
* **Bades.id UI 组件** — 从 `twenty-sdk/ui` 导入（Button、Tag、Status、Chip、Avatar 等）
