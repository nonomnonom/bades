---
title: 数据模型
description: 使用 Bades.id SDK 定义对象、字段、角色和应用程序元数据。
icon: database
---
...
<Note>
**基础字段会自动创建。** 当你定义自定义对象时，Bades.id 会自动添加标准字段
...
使用 `defineField()` 向你不拥有的对象添加字段——例如标准的 Bades.id 对象（Person、Company 等）。 或来自其他应用的对象。 与在 `defineObject()` 中的内联字段不同，独立字段需要一个 `objectUniversalIdentifier` 来指定它们要扩展的对象：
...
关系用于将对象彼此连接。 在 Bades.id 中，关系始终是双向的——你需要定义两侧，每一侧都引用另一侧。
...
要与内置的 Bades.id 对象（Person、Company 等）建立关系，请使用 `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`：
