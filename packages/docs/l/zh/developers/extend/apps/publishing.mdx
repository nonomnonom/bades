...
title: 发布
icon: 上传
description: 将你的 Bades.id 应用分发到应用市场，或进行内部部署。
---
...
* **部署 tar 包** — 直接将你的应用上传到特定的 Bades.id 服务器，以供内部或私有使用。
* **发布到 npm** — 将你的应用在 Bades.id 应用市场上架，供任何工作区发现并安装。
...
对于你不希望公开的应用（专有工具、仅供企业使用的集成或实验性构建），你可以将 tar 包直接部署到某台 Bades.id 服务器。
...
如果你的应用使用了特定 Bades.id 服务器版本中引入的功能（例如在 v2.3.0 中新增的 OAuth 提供方），应当在 `package.json` 的 `engines.twenty` 字段中声明应用所需的最低服务器版本：
...
使用 `create-bades-app` 生成的应用开箱即带有两个 GitHub Actions 工作流，位于 `.github/workflows/`。 当你将仓库推送到 GitHub 后即可运行——CI 无需额外设置，CD 只需要一个机密。
...
1. 检出你的应用源代码。
2. 使用 `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` 组合 action 启动一个隔离的 Bades.id 测试实例（相当于 CI 中的 `yarn twenty server start --test`）。
...
* `TWENTY_VERSION`（环境变量，默认 `latest`）— 通过在 `ci.yml` 中编辑它来固定 CI 使用的 Bades.id 服务器版本。
...
在每次向 `main` 推送时将你的应用部署到已配置的 Bades.id 服务器；当为拉取请求添加 `deploy` 标签时，也可从该拉取请求进行部署。
...
| 设置                      | 位置                                                       | 目的                                       |
| ----------------------- | -------------------------------------------------------- | ---------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `cd.yml` 中的 `env`（默认值为 `http://localhost:3000`）          | 要部署到的 Bades.id 服务器。 首次使用前将其更改为你真实的服务器 URL。 |
...
发布到 npm 可让你的应用在 Bades.id 应用市场中被发现。 任何 Bades.id 工作区都可以直接通过 UI 浏览、安装和升级应用市场中的应用。

### 要求

* 一个 [npm](https://www.npmjs.com) 账户
* 你在 `package.json` 的 `keywords` 数组中的 `twenty-app` 关键字（需要手动添加 — 在 `create-bades-app` 模板中默认不包含）
...
<Note>
如果您的应用未在 `defineApplication()` 中定义 `aboutDescription`，市场将自动使用 npm 上您的软件包的 `README.md` 作为关于页面内容。 这意味着您可以为 npm 和 Bades.id 市场维护同一个 README。 如果您希望在市场中使用不同的描述，请显式设置 `aboutDescription`。
</Note>
