...
该 SDK 提供可编程的 API，使你可以在测试代码中构建、部署、安装和卸载你的应用。 结合 [Vitest](https://vitest.dev/) 和类型化 API 客户端，你可以编写集成测试，在真实的 Bades.id 服务器上验证你的应用端到端运行是否正常。
...
<Note>
这与 `yarn twenty server logs` 不同，后者显示的是 Docker 容器日志。 `yarn twenty logs` 会显示来自 Bades.id 服务器的应用函数执行日志。
</Note>
...
“远程”是指你的应用连接到的 Bades.id 服务器。 在设置期间，脚手架工具会为你自动创建一个。 你可以随时添加更多远程或在它们之间切换。
...
1. 检出你的代码
2. 使用 `twentyhq/twenty/.github/actions/spawn-twenty-docker-image` 动作启动一个临时的 Bades.id 服务器
...
你无需配置任何机密——`spawn-twenty-docker-image` 动作会在运行器中直接启动一个临时的 Bades.id 服务器，并输出连接详情。 GitHub 会自动提供 `GITHUB_TOKEN` 机密。

若要固定为特定的 Bades.id 版本而不是 `latest`，请在工作流顶部修改 `TWENTY_VERSION` 环境变量。
