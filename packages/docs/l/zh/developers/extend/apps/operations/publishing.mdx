...
1. 检出你的应用源代码。
2. 使用 `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` 组合 action 启动一个隔离的 Bades.id 测试实例（相当于 CI 中的 `yarn twenty docker:start --test`）。
...
Bades.id 服务器会**每小时**从 npm 注册表同步其市场目录。
...
在 Bades.id 的 **Settings > Applications** 页面中，既可以浏览也可以安装来自市场或通过 tarball 部署的应用。
