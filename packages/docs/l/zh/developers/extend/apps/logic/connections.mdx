...
* 请使用 `serverVariables`（而非 `applicationVariables`）——OAuth 凭据是服务器范围的，并且每个 Bades.id 服务器只配置一个 OAuth 应用。
...
4. 以服务器管理员身份在 Bades.id 中打开已安装的应用 → 在相应的 `serverVariables` 上设置这些值。
