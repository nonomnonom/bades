...
icon: laptop-code
description: الدليل للمساهمين (أو المطورين الفضوليين) الذين يرغبون في تشغيل Bades.id محلياً.
---
...
قبل أن تتمكن من تثبيت واستخدام Bades.id، تأكد من تثبيت الأمور التالية على جهاز الكمبيوتر الخاص بك:
...
يتطلب Bades.id مخزن بيانات Redis لتقديم أفضل أداء.
...
## الخطوة الثامنة: استخدم Bades.id

**الواجهة الأمامية**

ستكون واجهة Bades.id الأمامية تعمل على  [http://localhost:3001](http://localhost:3001).
يمكنك تسجيل الدخول باستخدام حساب العرض التوضيحي الافتراضي: `tim@apple.dev` (كلمة المرور: `tim@apple.dev`)

**الخلفية**

* سيكون خادم Bades.id متصلاً ويعمل على [http://localhost:3000](http://localhost:3000)
