...
title: البدء
icon: rocket
description: أنشئ أول تطبيق Bades.id خلال دقائق.
---

## ما هي التطبيقات؟

تتيح لك التطبيقات توسيع Bades.id باستخدام كائنات وحقول مخصّصة ووظائف منطقية ومكوّنات الواجهة الأمامية ومهارات الذكاء الاصطناعي وغير ذلك — جميعها تُدار ككود. بدلًا من تكوين كل شيء عبر واجهة المستخدم، تعرّف نموذج بياناتك ومنطقك في TypeScript وتقوم بنشره إلى مساحة عمل واحدة أو أكثر.
...
* **Docker** — [نزّل من هنا](https://www.docker.com/products/docker-desktop/). مطلوب لتشغيل مثيل محلي من Bades.id. غير مطلوب إذا كان لديك خادم Bades.id قيد التشغيل بالفعل.
...
### إعداد مثيل محلي من Bades.id

ستسأل أداة إنشاء الهيكل:

> **هل ترغب في إعداد مثيل محلي من Bades.id؟**

* **اكتب `yes`** (موصى به) — سيؤدي ذلك إلى سحب صورة Docker `twenty-app-dev` وبدء تشغيل خادم Bades.id محلي على المنفذ `2020`. تأكّد من أن Docker قيد التشغيل قبل المتابعة.
* **اكتب `no`** — اختر هذا إذا كان لديك خادم Bades.id يعمل محليًا بالفعل.
...
بعد ذلك، ستُفتح نافذة متصفح تعرض صفحة تسجيل الدخول الخاصة بـ Bades.id. سجّل الدخول باستخدام حساب العرض التوضيحي المُجهَّز مسبقًا:

* **البريد الإلكتروني:** `tim@apple.dev`
* **كلمة المرور:** `tim@apple.dev`

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="شاشة تسجيل الدخول إلى Bades.id" />
</div>
...
<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="شاشة تفويض واجهة الأوامر (CLI) الخاصة بـ Bades.id" />
</div>
...
يقوم هذا بمراقبة ملفات المصدر لديك، وإعادة البناء عند كل تغيير، ومزامنة تطبيقك تلقائيًا مع خادم Bades.id المحلي. يفترض أن ترى لوحة حالة مباشرة في الطرفية.
...
<Warning>
وضع التطوير متاح فقط على مثيلات Bades.id التي تعمل في وضع التطوير (`NODE_ENV=development`). المثيلات الإنتاجية ترفض طلبات مزامنة وضع التطوير. استخدم `yarn twenty deploy` للنشر إلى خوادم الإنتاج — اطّلع على [نشر التطبيقات](/l/ar/developers/extend/apps/publishing) للتفاصيل.
</Warning>
...
كلا الوضعين يتطلبان خادم Bades.id يعمل في وضع التطوير وجهة بعيدة موثَّقة — تنطبق المتطلبات المسبقة نفسها.

### اعرض تطبيقك في Bades.id
...
انقر على **My twenty app** لفتح **تسجيل التطبيق** الخاص به. التسجيل عبارة عن سجل على مستوى الخادم يصف تطبيقك — اسمه، والمعرّف الفريد، وبيانات اعتماد OAuth، والمصدر (محلي، npm، أو tarball). يُخزَّن على الخادم، وليس داخل أي مساحة عمل محددة. عند تثبيت تطبيق في مساحة عمل، ينشئ Bades.id **تطبيقًا** بنطاق مساحة العمل يُشير مرة أخرى إلى هذا التسجيل. يمكن تثبيت تسجيل واحد عبر عدة مساحات عمل على الخادم نفسه.
...
| **المكوّنات الأمامية** | مكوّنات React تُعرَض داخل واجهة مستخدم Bades.id (اللوحة الجانبية، الودجات، قائمة الأوامر)           |
...
لقد قامت أداة إنشاء الهيكل بالفعل بتشغيل خادم Bades.id محليًا لك. لإدارته لاحقًا، استخدم `yarn twenty server`:
...
إذا كنت تفضّل إعداد الأمور بنفسك بدلًا من استخدام `create-bades-app`، فيمكنك ذلك بخطوتين.
...
ما زلت عالقًا؟ اطلب المساعدة على [خادم Bades.id على Discord](https://discord.com/channels/1130383047699738754/1130386664812982322).
