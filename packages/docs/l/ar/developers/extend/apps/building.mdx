---
title: الهيكلية
description: كيف تعمل تطبيقات Bades.id — العزل، دورة الحياة، واللبنات الأساسية.
icon: sitemap
---

تطبيقات Bades.id هي حزم TypeScript توسّع مساحة عملك بكائنات مخصّصة، ومنطق، ومكوّنات واجهة مستخدم (UI)، وقدرات ذكاء اصطناعي. تعمل على منصة Bades.id مع عزل كامل وضوابط الأذونات.
...
| **مكوّن أمامي**        | واجهة مستخدم React معزولة داخل صفحة Bades.id.            | [المكوّنات الأمامية](/l/ar/developers/extend/apps/front-components) |
...
* **المكوّنات الأمامية** تعمل ضمن Web Workers باستخدام Remote DOM — معزولة عن الصفحة الرئيسية لكنها تعرض عناصر DOM الأصلية (وليس iframes). تتواصل مع Bades.id عبر واجهة API للمضيف تعتمد تمرير الرسائل.
...
* **`yarn twenty dev`** — يراقب ملفات المصدر لديك ويزامن التغييرات مباشرةً إلى خادم Bades.id متصل. يُعاد توليد عميل API مضبوط الأنواع تلقائيًا عند تغيّر المخطط.
...
<CardGroup cols={٢}>
  ...
  <Card title="المكوّنات الأمامية" icon="window-maximize" href="/l/ar/developers/extend/apps/front-components">
    مكوّنات React معزولة داخل واجهة مستخدم Bades.id.
  </Card>
