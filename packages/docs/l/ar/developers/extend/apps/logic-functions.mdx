...
دوال المنطق هي دوال TypeScript على جانب الخادم تعمل على منصة Bades.id. يمكن تشغيلها بواسطة طلبات HTTP أو جداول cron أو أحداث قاعدة البيانات — كما يمكن إتاحتها كأدوات لوكلاء الذكاء الاصطناعي.
...
يمكن إتاحة الدوال المنطقية بوصفها **أدوات** لوكلاء الذكاء الاصطناعي وسير العمل. عند تمييز دالة كأداة، تصبح قابلة للاكتشاف بواسطة ميزات الذكاء الاصطناعي في Bades.id ويمكن استخدامها في أتمتة سير العمل.
...
* متغيرات البيئة `APPLICATION_ID` و`APP_ACCESS_TOKEN` و`API_URL` متاحة داخل المعالج (كما في أي دالة منطق أخرى)، لذا يمكنك استدعاء واجهة Bades.id API باستخدام رمز وصول للتطبيق مقيّد بنطاق تطبيقك.
...
توفر حزمة `twenty-client-sdk` عميلين لـ GraphQL ذوي أنواع ثابتة للتفاعل مع واجهة Bades.id البرمجية من وظائفك المنطقية ومكوّنات الواجهة الأمامية.
...
<Note>
  عند تشغيل كودك على Bades.id (وظائف منطقية أو مكوّنات أمامية)، يقوم النظام الأساسي بحقن بيانات الاعتماد كمتغيرات بيئية:

  * `TWENTY_API_URL` — عنوان URL الأساسي لواجهة Bades.id البرمجية
