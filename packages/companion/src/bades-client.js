...
const LOG_PREFIX = '[Bades]';

let configLogged = false;

function getApiUrl() {
  return process.env.BADES_API_URL;
}

function getWorkspaceSubdomain() {
  return process.env.BADES_WORKSPACE_SUBDOMAIN;
}

function getApiKey() {
  return process.env.BADES_API_KEY;
}
...
function isConfigured() {
  ...
  if (!configLogged) {
    ...
    if (!configured) {
      console.log(
        `${LOG_PREFIX} Integration disabled — BADES_API_URL or BADES_API_KEY not set`,
      );
    } else {
      console.log(
