resource "kubernetes_deployment" "bades_server" {
  metadata {
  ...
  spec {
    replicas = var.bades_server_replicas
    selector {
      match_labels = {
        app = "${var.bades_app_name}-server"
      }
    ...
    template {
      ...
      spec {
        container {
          ...
          volume_mount {
            name       = "server-data"
            mount_path = var.bades_server_data_mount_path
          }

          volume_mount {
            name       = "docker-data"
            mount_path = var.bades_docker_data_mount_path
          }
        }
