...
export const getChartLimitMessage = ({
  ...
  if (isPrimaryAxisDate && isDefined(primaryAxisDateGranularity)) {
    ...
    return `Data tidak ditampilkan: maks ${maxItems} ${granularityLabel} per grafik.`;
  }

  if (widgetConfigurationType === WidgetConfigurationType.LINE_CHART) {
    return `Data tidak ditampilkan: maks ${maxItems} titik data per grafik.`;
  }

  if (widgetConfigurationType === WidgetConfigurationType.BAR_CHART) {
    return `Data tidak ditampilkan: maks ${maxItems} batang per grafik.`;
  }

  return `Data tidak ditampilkan: maks ${maxItems} irisan per grafik.`;
};
