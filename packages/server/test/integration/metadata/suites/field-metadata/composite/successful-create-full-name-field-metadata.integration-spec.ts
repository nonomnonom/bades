...
const SUCCESSFUL_TEST_CASES: EachTestingContext<{
  ...
  {
    title:
      'full name field with basic metadata (name, label, description, icon)',
    context: {
      input: {
        name: 'contactName',
        label: 'Contact Name',
...
describe('Full name field metadata creation tests suite', () => {
  ...
  beforeAll(async () => {
    const { data } = await createOneObjectMetadata({
      expectToFail: false,
      input: {
        nameSingular: 'testFullNameObject',
        namePlural: 'testFullNameObjects',
        labelSingular: 'Test Full Name Object',
        labelPlural: 'Test Full Name Objects',
        icon: 'IconTestPipe',
