...
export const typeORMCoreModuleOptions: TypeOrmModuleOptions = {
  ...
  entities:
    process.env.IS_BILLING_ENABLED === 'true'
      ? [
          `${isJest ? 'src/' : 'dist/src/'}engine/core-modules/**/*.entity{.ts,.js}`,
          `${isJest ? 'src/' : 'dist/src/'}engine/metadata-modules/**/*.entity{.ts,.js}`,
        ]
      : [
          `${isJest ? 'src/' : 'dist/src/'}engine/core-modules/**/!(billing-*).entity.{ts,js}`,
          `${isJest ? 'src/' : 'dist/src/'}engine/metadata-modules/**/*.entity{.ts,.js}`,
        ],
  synchronize: false,
  migrationsRun: false,
  ...
  migrations:
    process.env.IS_BILLING_ENABLED === 'true'
      ? [
          `${isJest ? 'src/' : 'dist/src/'}database/typeorm/core/migrations/common/*{.ts,.js}`,
          `${isJest ? 'src/' : 'dist/src/'}database/typeorm/core/migrations/billing/*{.ts,.js}`,
        ]
      : [
          `${isJest ? 'src/' : 'dist/src/'}database/typeorm/core/migrations/common/*{.ts,.js}`,
        ],
