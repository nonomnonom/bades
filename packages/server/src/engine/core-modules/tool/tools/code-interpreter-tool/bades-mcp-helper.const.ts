...
export const BADES_MCP_HELPER = `# Auto-injected Bades MCP helper - provides access to Bades tools
import os
import json
...
class BadesMCP:
    ...
    def __init__(self):
        self.url = os.environ.get('BADES_SERVER_URL', '')
        self.token = os.environ.get('BADES_API_TOKEN', '')
    ...
    def call_tool(self, name: str, arguments: dict = None):
        ...
        Example:
            companies = bades.call_tool('find_companies', {'limit': 5})
...
# --------------------------------------------------------------------------
# \`bades\` is a pre-built instance of the BadesMCP class above. It is
# already bound in this module scope — DO NOT \`import bades\`. There is
# no Python package by that name. Just use it directly, e.g.:
#     companies = bades.call_tool('find_companies', {'limit': 10})
# --------------------------------------------------------------------------
bades = BadesMCP()
`;
