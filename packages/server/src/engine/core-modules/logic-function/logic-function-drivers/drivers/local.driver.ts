...
import { NODE_ESM_CJS_BANNER } from 'shared/application';
...
export class LocalDriver implements LogicFunctionDriver {
  ...
  private async ensureSdkLayer({
    ...
    await this.cacheLockService.withLock(
      async () => {
        ...
        const sdkPackagePath = join(sdkNodeModulesPath, 'client-sdk');
  ...
  // Symlinks everything from the deps layer except client-sdk,
  ...
  private async assembleNodeModules({
    ...
    const symlinkPromises = entries
      .filter((entry) => entry.name !== 'client-sdk')
      .map((entry) =>
        fs.symlink(
    ...
    await fs.symlink(
      join(sdkNodeModules, 'client-sdk'),
      join(execNodeModules, 'client-sdk'),
      'dir',
    );
  }
