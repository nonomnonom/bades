...
jest.mock(
  'src/engine/core-modules/bades-config/constants/config-variables-masking-config',
  () => ({
...
jest.mock(
  'src/engine/core-modules/bades-config/utils/is-env-only-config-var.util',
  () => ({
    isEnvOnlyConfigVar: jest.fn(),
  }),
);

type BadesConfigServicePrivateProps = {
...
const setupTestModule = async (isDatabaseConfigEnabled = true) => {
  ...
  return {
    service: module.get<BadesConfigService>(BadesConfigService),
...
const setPrivateProps = (
  service: BadesConfigService,
  props: Partial<BadesConfigServicePrivateProps>,
) => {
...
describe('BadesConfigService', () => {
  let service: BadesConfigService;
