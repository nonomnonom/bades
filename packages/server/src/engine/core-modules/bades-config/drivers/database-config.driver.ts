...
import { type DatabaseConfigDriverInterface } from 'src/engine/core-modules/bades-config/drivers/interfaces/database-config-driver.interface';

import { ConfigCacheService } from 'src/engine/core-modules/bades-config/cache/config-cache.service';
import { ConfigVariables } from 'src/engine/core-modules/bades-config/config-variables';
import { CONFIG_VARIABLES_REFRESH_CRON_INTERVAL } from 'src/engine/core-modules/bades-config/constants/config-variables-refresh-cron-interval.constants';
import { ConfigStorageService } from 'src/engine/core-modules/bades-config/storage/config-storage.service';
  ...
  implements DatabaseConfigDriverInterface, OnModuleInit
{
  ...
  async set<T extends keyof ConfigVariables>(
    ...
    if (isEnvOnlyConfigVar(key)) {
      throw new Error(
        `Tidak dapat menetapkan variabel environment-only: ${key as string}`,
      );
    }
