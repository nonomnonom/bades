...
import { WorkspaceSoftDeleteQueryBuilder } from 'src/engine/sid-orm/repository/workspace-soft-delete-query-builder';
import { WorkspaceUpdateQueryBuilder } from 'src/engine/sid-orm/repository/workspace-update-query-builder';
...
> extends SelectQueryBuilder<T> {
  ...
  override async execute(): Promise<any> {
    try {
      ...
      throw await computeSidOrmException(error);
    }
  }
  ...
  private getMainAliasTarget(): EntityTarget<T> {
    ...
    if (!mainAliasTarget) {
      throw new SidOrmException(
        ...
        SidOrmExceptionCode.MISSING_MAIN_ALIAS_TARGET,
      );
    }
