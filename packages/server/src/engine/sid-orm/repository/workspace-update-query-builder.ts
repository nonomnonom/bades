...
import { type WorkspaceDeleteQueryBuilder } from 'src/engine/sid-orm/repository/workspace-delete-query-builder';
...
> extends UpdateQueryBuilder<T> {
    ...
    try {
      ...
      if (before.length > QUERY_MAX_RECORDS) {
        throw new SidOrmException(
          ...
          SidOrmExceptionCode.TOO_MANY_RECORDS_TO_UPDATE,
          {
  ...
  override select(): WorkspaceSelectQueryBuilder<T> {
    throw new SidOrmException(
      ...
      SidOrmExceptionCode.METHOD_NOT_ALLOWED,
    );
  }
