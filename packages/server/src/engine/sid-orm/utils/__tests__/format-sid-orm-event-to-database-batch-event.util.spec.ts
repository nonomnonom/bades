import { type ObjectRecordUpdateEvent } from 'shared/database-events';
...
describe('formatTwentyOrmEventToDatabaseBatchEvent', () => {
  ...
  describe('UPDATED action', () => {
    it('should throw SidOrmException when no matching before entity is found in array of beforeEntities', () => {
      const afterEntities = [
        {
          id: 'record-1',
      ...
      try {
        ...
        expect(error).toBeInstanceOf(SidOrmException);
        expect((error as SidOrmException).code).toBe(
          SidOrmExceptionCode.ORM_EVENT_DATA_CORRUPTED,
        );
        expect((error as SidOrmException).message).toBe(
